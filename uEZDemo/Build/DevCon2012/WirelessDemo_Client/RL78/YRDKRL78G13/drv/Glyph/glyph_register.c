...
void LCD_SPI_CommandSend(int8_t aCommand)
{
    ...
    SPI_Transfer(SPI_LCD_CHANNEL, 1, (uint8_t *)&aCommand, &dummyRX, 0);
    while(SPI_IsBusy(SPI_LCD_CHANNEL)); // Wait for transmission end
...
void LCD_SPI_DataSend(int8_t aData)
{
    ...
    SPI_Transfer(SPI_LCD_CHANNEL, 1, (uint8_t *)&aData, &dummyRX, 0);
    while(SPI_IsBusy(SPI_LCD_CHANNEL)); // Wait for transmission end
}
