 ...
 * File:  App_Connect.c
 *-------------------------------------------------------------------------*/
 /** @addtogroup App_Connect
 * @{
 *     @brief Routines for connecting to the network.  Usually done at power up.
 */
/*-------------------------------------------------------------------------*/
 ...
 * Routine:  App_Connect
 *---------------------------------------------------------------------------*/
 /** Connects to network using supplied settings
 *
 * @param [in] wp -- pointer to web provisioning settings
 * @return    void
 */
 /*---------------------------------------------------------------------------*/
 ATLIBGS_MSG_ID_E App_Connect(ATLIBGS_WEB_PROV_SETTINGS *wp)
{
    ...
    while (1) {
        ...
        if (wp->dhcp_enable) {
            ...
            // Setup the IP address
            //AtLibGs_IPv4AddressToString(&wp->ip, ip);
            strcpy(ip, "192.168.10.2");
            //AtLibGs_IPv4AddressToString(&wp->subnet, subnet);
            strcpy(subnet, "255.255.255.0");
            //AtLibGs_IPv4AddressToString(&wp->gateway, gateway);
            strcpy(gateway, "192.168.10.1");
            rxMsgId = AtLibGs_IPSet(ip, subnet, gateway);
            if (rxMsgId != ATLIBGS_MSG_ID_OK) {
                DisplayLCD(LCD_LINE8, "Bad IP!");
                MSTimerDelay(2000);
