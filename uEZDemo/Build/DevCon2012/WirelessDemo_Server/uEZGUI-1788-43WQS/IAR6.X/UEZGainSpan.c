...
// Redirect FDI Cmd prints to standard printf
#define FDICmdPrintf(ws, ...)  printf(__VA_ARGS__)
#define FDICmdSendString(ws, str) printf("%s", str)
...
typedef enum {
    TEMP_STATE_MODE_LOOKING_FOR_COLON=0,
    TEMP_STATE_MODE_LOOKING_FOR_CR=1,
    TEMP_STATE_MODE_LOOKING_FOR_LF=2,
} T_tempStateMode;
...
T_uezSemaphore G_tempStatusSem;
T_GainSpanTemperatureStatus G_tempStatus;
...
T_uezError UEZGUIGainSpanAdHoc1(void *aWorkspace)
{
    ...
    if (AtLibGs_Assoc("FDI_10172012", 0, 0) == ATLIBGS_MSG_ID_OK) {
...
T_uezError UEZGUIGainSpanLAP1(void *aWorkspace)
{
    ...
    AtLibGs_CalcNStorePSK("FDI_10172012", "fdifdifdiFDI");
...
int UEZGUIGainSpan(void *aWorkspace, int argc, char *argv[], char *rxBuf)
{
...
#if CONFIG_USE_EXP4
    const T_GainSpan_CmdLib_SPISettings settings = { "SSP2", 1000000, // 1 MHz

        GPIO_P1_8, // iSPIChipSelect = WIFI SPI CS
        GPIO_P0_5, // iDataReadyIO = WIFI IRQ
        GPIO_P1_14, // iProgramMode = WIFI PROGRAM OFF
        GPIO_P1_15, // iSPIMode = WIFI_MODE SPI
        GPIO_P1_16, // iSRSTn = WIFI_SRSTn;
    ...
    const T_GainSpan_CmdLib_SPISettings settings = { "SSP0", 1000000, // 1 MHz
        ...
        GPIO_P2_23, // iSPIChipSelect = WIFI SPI CS
        GPIO_P2_11, // iDataReadyIO = WIFI IRQ
        GPIO_P1_6, // iProgramMode = WIFI PROGRAM OFF
        GPIO_P1_7, // iSPIMode = WIFI_MODE SPI
        GPIO_P1_5, // iSRSTn = WIFI_SRSTn;
    ...
    if (UEZSPIOpen("SSP0", &SPI) == UEZ_ERROR_NONE) {
        ...
        if (argc == 1) { // SPI Ping Test, not using AT command, this is legacy
            while (timeout < 1000) {
                // Is Data Ready?
                if (UEZGPIORead(GPIO_P2_11) == ETrue) {
...
void ITempServerHandleLine(char *aLine)
{
    UEZSemaphoreGrab(G_tempStatusSem, UEZ_TIMEOUT_INFINITE);
    sscanf(aLine, "%d %d", &G_tempStatus.iTemperature, &G_tempStatus.iPotentiometer);
    UEZSemaphoreRelease(G_tempStatusSem);
}

void ITempServerParseChar(T_tempServerState *p_state, char c)
{
    if (c == ':') {
        p_state->iNum = 0;
        p_state->iLine[0] = ':';
        ...
        // End of line!
        // Process the line if we have a colon at the start
        if ((p_state->iNum > 0) && (p_state->iLine[0] == ':')) {
            // Process the line!
            ITempServerHandleLine(p_state->iLine+1);
            p_state->iNum = 0;
        }
    } else {
        // Have we received at least one colon?
        if ((p_state->iNum > 0) && (p_state->iLine[0] == ':')) {
            // Do we have room?
            if (p_state->iNum < 80-1) {
                // Append this character
                p_state->iLine[p_state->iNum++] = c;
                p_state->iLine[p_state->iNum] = '\0';;
            }
        }
    }
}

void ITempServerParseChars(T_tempServerState *p_state, ATLIBGS_TCPMessage *msg)
{
    TUInt32 i=0;
    for (i=0; i<msg->numBytes; i++)
        ITempServerParseChar(p_state, msg->message[i]);
}

T_uezError UEZGUIGainSpanTemperatureServer(void *aWorkspace)
{
    bool connected = false;
    ATLIBGS_TCPConnection connection;
    ATLIBGS_TCPMessage msg;
    ...
    ATLIBGS_MSG_ID_E rxMsgId;
    TUInt32 timeout;
    T_tempServerState state;
    state.iNum = 0;
    state.iMode = 0;
//    TBool active = EFalse;

    // Initialize temperature status
    G_tempStatus.iTemperature = TEMPERATURE_NO_READING;
    G_tempStatus.iPotentiometer = 0;

    // Setup temperature status semaphore
    UEZSemaphoreCreateBinary(&G_tempStatusSem);
    ...
    if (rxMsgId != ATLIBGS_MSG_ID_OK) {
        ...
        while (1) {
            ...
            if (connected) {
                if (AtLibGs_WaitForTCPMessage(250) == ATLIBGS_MSG_ID_DATA_RX) {
                    AtLibGs_ParseTCPData(G_received, G_receivedCount, &msg);
                    ITempServerParseChars(&state, &msg);
#if 0
                    //FDICmdPrintf(aWorkspace, "Data: [%s]\n", msg.message);
                    if (msg.message[0] == '0') {
                        FDICmdPrintf(aWorkspace, "OFF\n");
...
//                        active = ETrue;
                    ...
                    if (timeout >= 5000) {
                        // No communications for 5 seconds!
                        // Use a low value for temperature and pot
                        UEZSemaphoreGrab(G_tempStatusSem, UEZ_TIMEOUT_INFINITE);
                        G_tempStatus.iTemperature = -10000;
                        G_tempStatus.iPotentiometer = 0;
                        UEZSemaphoreRelease(G_tempStatusSem);
...
{
    UEZSemaphoreGrab(G_tempStatusSem, UEZ_TIMEOUT_INFINITE);
    *aStatus = G_tempStatus;
    UEZSemaphoreRelease(G_tempStatusSem);
}

TUInt32 UEZGUIGainSpanTask(T_uezTask aMyTask, void *aParameters)
{
...
//    ATLIBGS_MSG_ID_E rxMsgId;
    ...
    if (UEZSPIOpen("SSP0", &SPI) == UEZ_ERROR_NONE) {
        ...
        UEZGUIGainSpanTemperatureServer(0);
    }
...
{
    UEZTaskCreate(
        (T_uezTaskFunction)UEZGUIGainSpanTask,
        "Wireless",
        UEZ_TASK_STACK_BYTES(4096),
        (void *)0,
        UEZ_PRIORITY_NORMAL,
        0);
}
