...
static GUI_AUTODEV G_Image1_MEMDev;
...
static TBool IHandleSettings(WM_MESSAGE * pMsg, int aNCode, int aID)
{
    /*if (aNCode == WM_NOTIFICATION_RELEASED) {
        WindowManager_Show_Window_PF(SETTINGS_SCREEN);
    }*/
    return EFalse;
}
...
#if 0
    if (c.ClipRect.x0 > 0) {
        GUI_MEMDEV_WriteAt(G_Current, 0, 0);
    }
#else
...
static void _PFMainDialog(WM_MESSAGE *pMsg)
{
    ...
    case WM_INIT_DIALOG:
        ...
        GUI_MEMDEV_CreateAuto(&G_Image1_MEMDev);
        GUI_MEMDEV_DrawAuto(&G_Image1_MEMDev, &Param.AutoDevInfo,
                &_Draw, &Param);
