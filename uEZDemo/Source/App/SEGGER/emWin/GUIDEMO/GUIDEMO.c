...
#if SHOW_GUIDEMO_SPEEDOMETER
  GUIDEMO_Speedometer,
#endif
#if SHOW_GUIDEMO_ZOOMANDROTATE
  GUIDEMO_ZoomAndRotate,
#endif
#if SHOW_GUIDEMO_SKINNING
  GUIDEMO_Skinning,
#endif
#if SHOW_GUIDEMO_FADING
  GUIDEMO_Fading,
#endif
...
static void _Main(void) {
  ...
  // Run the demos
  //
  for (_iDemo = 0; _apfTest[_iDemo]; _iDemo++) {
    _ClearHalt();
...
void GUIDEMO_UpdateControlText(void) {
  ...
  GUIDEMO_AddStringToString(acText, "/");
  GUIDEMO_AddIntToString   (acText, GUI_COUNTOF(_apfTest) - 1);
