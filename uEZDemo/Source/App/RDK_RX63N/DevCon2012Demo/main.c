...
static T_httpServerParameters G_HTTPParams = { MyHTTPServerGetValue, MyHTTPServerSetValue };
...
void DataLogXYZ(double x, double y, double z)
{
    ...
    if (UEZFileOpen("0:/HTTPROOT/ACCELLOG.TXT", FILE_FLAG_APPEND, &file) == UEZ_ERROR_NONE) {
...
{	
    ...
    PORTC.PDR.BIT.B2 = 1; // LCD-CS
    PORTC.PODR.BIT.B2 = 0; // LCD-CS set Low (inactive)
