 ...
 *---------------------------------------------------------------------------*/
/**
 *  Console command "LCD" that uses the glyph API to clear the screen and
 *  put characters on the LCD display.  Command is "LCD <string>".  The
 *  <string> can only be up to 12 characters.
 *
 *  @param [in]     *aWorkspace Workspace for this command console
 *  @param [in]     argc        Number of arguments to this command
 *  @param [in]     argv        List of arguments including command
...
int CmdLCD(void *aWorkspace, int argc, char *argv[])
{
    // Check that the command is of the form "LCD <string>"
    if (argc == 2) {
        // Clear the screen, go to the upper left, and output the string
        GlyphClearScreen(G_glyphLCD);
 ...
 *---------------------------------------------------------------------------*/
/**
 *  Console command "DIR" that uses the uEZ File system to show the files
 *  in the directory "0:/HTTPROOT".
 *
 *  @param [in]     *aWorkspace Workspace for this command console
 *  @param [in]     argc        Number of arguments to this command
 *  @param [in]     argv        List of arguments including command
...
int CmdDir(void *aWorkspace, int argc, char *argv[])
{
    ...
    // Start access to the file list at 0:/HTTPROOT
    if (UEZFileFindStart("0:/HTTPROOT", &entry) == UEZ_ERROR_NONE) {
        do {
            // Output each file or directory in the list
            ...
            // Go to the next directory entry and stop if there is no more
        } while (UEZFileFindNext(&entry) == UEZ_ERROR_NONE);
    } else {
        FDICmdPrintf(aWorkspace, "No files found.\n");
    }
    // End access to the file directory entries
 ...
 *---------------------------------------------------------------------------*/
/**
 *  Log the data out to the serial port and log file.  Keep a copy for the
 *  web server to use.
 *
