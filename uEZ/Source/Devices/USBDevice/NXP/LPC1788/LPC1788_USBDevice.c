 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      Device implementation of the NXP LPC1788 USBDevice Bus.
 *-------------------------------------------------------------------------*/
...
} T_USBDevice_LPC1788_Workspace;

/*---------------------------------------------------------------------------*
 * Prototypes:
 *---------------------------------------------------------------------------*/
static TBool NXP_LPC1788_USBDevice_StandardDeviceRequest(
                void *aWorkspace,
                T_USBSetupPacket *aSetup,
                TUInt16 *aLength,
                TUInt8 **aData);
static void INXP_LPC1788_StandardControlTransfer(
        void *aWorkspace,
        TUInt8 aEndpoint,
        T_USBEndpointStatus aStatus);
static void INXP_LPC1788_USBDevice_EndpointReady(void *aCallbackWorkspace);
...
static T_uezError USBDevice_NXP_InitializeWorkspace_USBDevice(void *aWorkspace)
{
    ...
    T_USBDevice_LPC1788_Workspace *p = (T_USBDevice_LPC1788_Workspace *)aWorkspace;
    ...
    p->iUSBDevice.iRequestTypeHandlers[USB_REQUEST_TYPE_STANDARD].iHandler =
            NXP_LPC1788_USBDevice_StandardDeviceRequest;
    p->iUSBDevice.iRequestTypeHandlers[USB_REQUEST_TYPE_STANDARD].iWorkspace =
            aWorkspace;
    p->iUSBDevice.iStatusEndpointReadyFunc = INXP_LPC1788_USBDevice_EndpointReady;
    p->iUSBDevice.iStatusChangeFunc = 0; // undefined

    // These are ALWAYS set to the standard control transfer at first
    p->iUSBDevice.iEndpointFuncs[0].iIn = INXP_LPC1788_StandardControlTransfer;
    p->iUSBDevice.iEndpointFuncs[0].iOut = INXP_LPC1788_StandardControlTransfer;
 ...
 *      into 64 byte blocks.
 * Inputs:
 *      T_USBDevice_LPC1788_Workspace *p -- Workspace
 *-------------------------------------------------------------------------*/
static void IUSBCreateDataInPacket(T_USBDevice_LPC1788_Workspace *p)
{
...
static TBool ICallSetupHandler(
                T_USBDevice_LPC1788_Workspace *p,
                T_USBSetupPacket *aSetup,
                TUInt16 *aLength,
                TUInt8 **aData)
{
...
/*-------------------------------------------------------------------------*
 * Function:  INXP_LPC1788_StandardControlTransfer
 *-------------------------------------------------------------------------*
 * Description:
        ...
        T_USBEndpointStatus aStatus)
{
    ...
    TUInt8 setupType;
    T_USBDevice_LPC1788_Workspace *p =
        (T_USBDevice_LPC1788_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  INXP_LPC1788_USBDevice_EndpointReady
 *---------------------------------------------------------------------------*
 * Description:
...
{
    T_USBDevice_LPC1788_Workspace *p =
        (T_USBDevice_LPC1788_Workspace *)aCallbackWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  NXP_LPC1788_USBDevice_ProcessEndpoints
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
static T_uezError NXP_LPC1788_USBDevice_ProcessEndpoints(
        void *aWorkspace,
        TUInt32 aTimeout)
{
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_Connect
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_Connect(void *aWorkspace)
{
    T_USBDevice_LPC1788_Workspace *p =
        (T_USBDevice_LPC1788_Workspace *)aWorkspace;
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_Disconnect
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- USB Device's workspace
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_Disconnect(void *aWorkspace)
{
    T_USBDevice_LPC1788_Workspace *p =
        (T_USBDevice_LPC1788_Workspace *)aWorkspace;
    UEZSemaphoreGrab(p->iSem, UEZ_TIMEOUT_INFINITE);
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_SetAddress
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_SetAddress(void *aWorkspace, TUInt8 aAddress)
{
    T_USBDevice_LPC1788_Workspace *p =
        (T_USBDevice_LPC1788_Workspace *)aWorkspace;
    UEZSemaphoreGrab(p->iSem, UEZ_TIMEOUT_INFINITE);
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_EndpointConfigure
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_EndpointConfigure(
            void *aWorkspace,
            TUInt8 aEndpoint,
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_Read
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static TInt16 NXP_LPC1788_USBDevice_Read(
            void *aWorkspace,
            TUInt8 aEndpoint,
            TUInt8 *aData,
            TUInt16 iMaxLen)
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_Write
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *      TUInt16                     -- aLength
 *-------------------------------------------------------------------------*/
static TUInt16 NXP_LPC1788_USBDevice_Write(
            void *aWorkspace,
            TUInt8 aEndpoint,
            TUInt8 *aData,
            TUInt16 iMaxLen)
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_SetStallState
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *                                     not stalled.
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_SetStallState(
            void *aWorkspace,
            TUInt8 aEndpoint,
            TBool aStallState)
{
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_IsStalled
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static TBool NXP_LPC1788_USBDevice_IsStalled(void *aWorkspace, TUInt8 aEndpoint)
{
    TBool result;
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_EndpointInterruptsEnable
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_EndpointInterruptsEnable(
                void *aWorkspace,
                TUInt8 aEndpoint)
{
    T_USBDevice_LPC1788_Workspace *p =
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_EndpointInterruptsDisable
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *      TUInt8 aEndpoint            -- Endpoint to trigger interrupts
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_EndpointInterruptsDisable(
                void *aWorkspace,
                TUInt8 aEndpoint)
{
    T_USBDevice_LPC1788_Workspace *p =
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_ForceAddressAndConfiguration
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_ForceAddressAndConfiguration(
            void *aWorkspace,
            TUInt8 aAddress,
            TUInt8 aConfigNum)
{
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_Initialize
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- USB Device's workspace
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_Initialize(void *aWorkspace)
{
    T_USBDevice_LPC1788_Workspace *p =
        (T_USBDevice_LPC1788_Workspace *)aWorkspace;
    UEZSemaphoreGrab(p->iSem, UEZ_TIMEOUT_INFINITE);
...
/*---------------------------------------------------------------------------*
 * Routine:  NXP_LPC1788_USBDevice_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
static T_uezError NXP_LPC1788_USBDevice_Configure(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  NXP_LPC1788_USBDevice_RegisterEndpointCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                      successful.
 *---------------------------------------------------------------------------*/
static T_uezError NXP_LPC1788_USBDevice_RegisterEndpointCallback(
            void *aWorkspace,
            TUInt8 aEndpoint,
...
/*---------------------------------------------------------------------------*
 * Routine:  NXP_LPC1788_USBDevice_UnregisterEndpointCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                      successful.
 *---------------------------------------------------------------------------*/
static T_uezError NXP_LPC1788_USBDevice_UnregisterEndpointCallback(
            void *aWorkspace,
            TUInt8 aEndpoint)
{
    T_uezError error = UEZ_ERROR_NONE;
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_StandardDeviceRequest
 *-------------------------------------------------------------------------*
 * Description:
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_RegisterRequestTypeCallback
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *                                  type.
 *-------------------------------------------------------------------------*/
static T_uezError NXP_LPC1788_USBDevice_RegisterRequestTypeCallback(
            void *aWorkspace,
            TUInt8 aType,
            void *aBuffer,
            ...
            T_USBRequestHandlerFunc aRequestHandler)
{
    ...
    if (aType <= USB_REQUEST_TYPE_RESERVED) {
        ...
        if ((p_handler->iHandler == 0) ||
                ((p_handler->iHandler == NXP_LPC1788_USBDevice_StandardDeviceRequest) &&
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_UnregisterRequestTypeCallback
 *-------------------------------------------------------------------------*
 * Description:
 *      Register a handler for different types of control 0 requests.
 * Inputs:
 ...
 *                                  type.
 *-------------------------------------------------------------------------*/
static T_uezError NXP_LPC1788_USBDevice_UnregisterRequestTypeCallback(
            void *aWorkspace,
            TUInt8 aType)
{
    T_uezError error = UEZ_ERROR_NONE;
...
/*-------------------------------------------------------------------------*
 * Function:  NXP_LPC1788_USBDevice_InterruptNakEnable
 *-------------------------------------------------------------------------*
 * Description:
 ...
 *-------------------------------------------------------------------------*/
static void NXP_LPC1788_USBDevice_InterruptNakEnable(
                void *aWorkspace,
                TUInt8 aNakBits)
{
    T_USBDevice_LPC1788_Workspace *p =
...
void USBDevice_LPC1788_Create(const char *aName, const char *aHALName)
{
    T_USBDevice_LPC1788_Workspace *p;
    ...
    UEZDeviceTableRegister(
            aName,
            (T_uezDeviceInterface *)&USBDevice_NXP_LPC1788_Interface,
            0,
 ...
 *---------------------------------------------------------------------------*/
const DEVICE_USB_DEVICE USBDevice_NXP_LPC1788_Interface = {
	{
	    // Common device interface
	    "USBDevice:NXP:LPC1788",
	    0x0100,
	    USBDevice_NXP_InitializeWorkspace_USBDevice,
	    sizeof(T_USBDevice_LPC1788_Workspace),
	},
	
    // Functions
    NXP_LPC1788_USBDevice_Configure,
    NXP_LPC1788_USBDevice_Initialize,
    NXP_LPC1788_USBDevice_Connect,
    NXP_LPC1788_USBDevice_Disconnect,
    NXP_LPC1788_USBDevice_SetAddress,
    NXP_LPC1788_USBDevice_EndpointConfigure,
    NXP_LPC1788_USBDevice_Read,
    NXP_LPC1788_USBDevice_Write,
    NXP_LPC1788_USBDevice_SetStallState,
    NXP_LPC1788_USBDevice_IsStalled,
    NXP_LPC1788_USBDevice_EndpointInterruptsEnable,
    NXP_LPC1788_USBDevice_EndpointInterruptsDisable,
    NXP_LPC1788_USBDevice_ForceAddressAndConfiguration,
    NXP_LPC1788_USBDevice_RegisterEndpointCallback,
    NXP_LPC1788_USBDevice_UnregisterEndpointCallback,
    NXP_LPC1788_USBDevice_RegisterRequestTypeCallback,
    NXP_LPC1788_USBDevice_UnregisterRequestTypeCallback,
    NXP_LPC1788_USBDevice_ProcessEndpoints,
    NXP_LPC1788_USBDevice_InterruptNakEnable,
} ;
