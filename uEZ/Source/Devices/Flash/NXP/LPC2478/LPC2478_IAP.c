...
const T_sector G_sectorMemory[MAX_NUM_SECTORS] = {
#if (LPC2478_IAP_PROTECT_FIRST_64K==1)
    ...
    {   8,  0x00008000, 0x00008000 },
#endif    
    {   9,  0x00010000, 0x00008000 },
    {  10,  0x00018000, 0x00008000 },
    {  11,  0x00020000, 0x00008000 },
    {  12,  0x00028000, 0x00008000 },
    {  13,  0x00030000, 0x00008000 },
    {  14,  0x00038000, 0x00008000 },
    {  15,  0x00040000, 0x00008000 },
    {  16,  0x00048000, 0x00008000 },
    {  17,  0x00050000, 0x00008000 },
    {  18,  0x00058000, 0x00008000 },
    {  19,  0x00060000, 0x00008000 },
    {  20,  0x00068000, 0x00008000 },
    {  21,  0x00070000, 0x00008000 },
    {  22,  0x00078000, 0x00001000 },
    {  23,  0x00079000, 0x00001000 },
    {  24,  0x0007A000, 0x00001000 },
    {  25,  0x0007B000, 0x00001000 },
    {  26,  0x0007C000, 0x00001000 },
    {  27,  0x0007D000, 0x00001000 },
...
__interwork __thumb static TIAPResult IIAPCmd(TUInt8 aCmd)
#else
#error "What is the proper way to put into THUMB mode?"
#endif
{
    // Pass on the command
    G_parameters[0] = aCmd;
...
static T_uezError IIAPBlankCheck(TUInt32 aSector, TBool *aIsBlank)
{
    TIAPResult result;
    ...
    G_parameters[3] = 0;
    result = IIAPCmd(IAP_CMD_BLANK_CHECK);
...
static T_uezError IIAPCmdErase(TUInt32 aSector)
{
    ...
    if (IIAPBlankCheck(aSector, &isBlank) != UEZ_ERROR_NONE)
        return UEZ_ERROR_COULD_NOT_ERASE;
 ...
 *---------------------------------------------------------------------------*/
T_uezError ILPC2478_IAP_Start(TBool aNeedErasing)
{
    TUInt32 i;
 ...
 *      T_uezError -- UEZ_ERROR_NONE if successful, else error code.
 *---------------------------------------------------------------------------*/
T_uezError ILPC2478_IAP_End(void)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup of this workspace for Data Flash. For the NXP LPC2478, we allow the MCU to
 ...
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_InitializeWorkspace(void *aW)
{
    T_uezError error;

    T_Flash_NXP_LPC2478_Workspace *p = (T_Flash_NXP_LPC2478_Workspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_Open(void *aWorkspace)
{
    T_Flash_NXP_LPC2478_Workspace *p =
        (T_Flash_NXP_LPC2478_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_Close
 *---------------------------------------------------------------------------*
 * Description:
 *      Close the flash device.  For the NXP LPC2478, the flash area access
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_Close(void *aWorkspace)
{
    T_Flash_NXP_LPC2478_Workspace *p =
        (T_Flash_NXP_LPC2478_Workspace *)aWorkspace;
    if (p->iNumOpen) {
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_Read
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_Read(
    void *aWorkspace,
    TUInt32 aOffset,
    TUInt8 *aBuffer,
    TUInt32 aNumBytes)
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_Write
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_Write(
    void *aWorkspace,
    TUInt32 aOffset,
    TUInt8 *aBuffer,
    TUInt32 aNumBytes)
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_BlockErase
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_BlockErase(
    void *aWorkspace,
    TUInt32 aOffset,
    TUInt32 aNumBytes)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_ChipErase
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_ChipErase(void *aWorkspace)
{
    T_Flash_NXP_LPC2478_Workspace *p =
        (T_Flash_NXP_LPC2478_Workspace *)aWorkspace;
    T_uezError error;
    ...
    // Prepare the sector for erase
    UEZSemaphoreGrab(p->iSem, UEZ_TIMEOUT_INFINITE);
#if LPC2478_IAP_PROTECT_FIRST_64K
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_QueryReg
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_QueryReg(
    void *aWorkspace,
    TUInt32 aReg,
    TUInt32 *aValue)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_GetChipInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_GetChipInfo(
    void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  Flash_NXP_LPC2478_GetBlockInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError Flash_NXP_LPC2478_GetBlockInfo(
    void *aWorkspace,
    TUInt32 aOffset,
...
void Flash_NXP_LPC2478_Create(const char *aName)
{
    UEZDeviceTableRegister(aName,
        (T_uezDeviceInterface *)&Flash_NXP_LPC2478_Interface, 0, 0);
}

/*---------------------------------------------------------------------------*
 * Device Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_Flash Flash_NXP_LPC2478_Interface = { {
// Common device interface
    "Flash:NXP:LPC2478:16Bit",
    0x0100, //<<---- What is the 0x100 stand for??
    Flash_NXP_LPC2478_InitializeWorkspace,
    sizeof(T_Flash_NXP_LPC2478_Workspace), },

// Functions
    Flash_NXP_LPC2478_Open,
    Flash_NXP_LPC2478_Close,
    Flash_NXP_LPC2478_Read,
    Flash_NXP_LPC2478_Write,
    Flash_NXP_LPC2478_BlockErase,
    Flash_NXP_LPC2478_ChipErase,
    Flash_NXP_LPC2478_QueryReg,
    Flash_NXP_LPC2478_GetChipInfo,
    Flash_NXP_LPC2478_GetBlockInfo, };

/*-------------------------------------------------------------------------*
 * End of File:  LPC2478_IAP.c
 *-------------------------------------------------------------------------*/
