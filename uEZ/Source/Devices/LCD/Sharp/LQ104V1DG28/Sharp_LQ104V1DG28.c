/*-------------------------------------------------------------------------*
 * File:  Sharp_LQ104V1DG28.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the Sharp LQ104V1DG28.
 *-------------------------------------------------------------------------*/
...
#ifndef LCD_SHARP_LQ104V1DG28_FLIP_SCREEN_X
#define LCD_SHARP_LQ104V1DG28_FLIP_SCREEN_X       0
#endif

#ifndef LCD_SHARP_LQ104V1DG28_FLIP_SCREEN_Y
#define LCD_SHARP_LQ104V1DG28_FLIP_SCREEN_Y       0
#endif

#define LCD_CLOCK_RATE      25000000
...
typedef struct {
    ...
    T_uezGPIOPortPin iStandByControlPin;
	T_uezSemaphore iVSyncSem;
} T_LQ104V1DG28Workspace;


/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_LQ104V1DG28_settings;

static const T_LCDControllerSettings LCD_LQ104V1DG28_params16bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_16_565,

    14,         /* Horizontal back porch */
    104,         /* Horizontal front porch */
...
static const T_LCDControllerSettings LCD_LQ104V1DG28_paramsI15bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_LQ104V1DG28_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_LQ104V1DG28_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_LQ104V1DG28_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_LQ104V1DG28_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_SHARP_LQ104V1DG28_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LQ104V1DG28 LCD.
 * Inputs:
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LQ104V1DG28 LCD.
 * Inputs:
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LQ104V1DG28 LCD.
 * Inputs:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_Close(void *aW)
{
    T_LQ104V1DG28Workspace *p = (T_LQ104V1DG28Workspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_LQ104V1DG28_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_LQ104V1DG28Workspace *p = (T_LQ104V1DG28Workspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_LQ104V1DG28Workspace *p = (T_LQ104V1DG28Workspace *)aW;
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_On(void *aW)
{
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_Off(void *aW)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_LQ104V1DG28Workspace *p = (T_LQ104V1DG28Workspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_LQ104V1DG28Workspace *p = (T_LQ104V1DG28Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ104V1DG28_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ104V1DG28_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_LQ104V1DG28Workspace *p = (T_LQ104V1DG28Workspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_LQ104V1DG28_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_SHARP_LQ104V1DG28_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:SHARP_LQ104V1DG28:16Bit",
	    0x0100,
	    LCD_LQ104V1DG28_InitializeWorkspace_16Bit,
	    sizeof(T_LQ104V1DG28Workspace),
	},
	
    // Functions
    LCD_LQ104V1DG28_Open,
    LCD_LQ104V1DG28_Close,
    LCD_LQ104V1DG28_Configure,
    LCD_LQ104V1DG28_GetInfo,
    LCD_LQ104V1DG28_GetFrame,
    LCD_LQ104V1DG28_ShowFrame,
    LCD_LQ104V1DG28_On,
    LCD_LQ104V1DG28_Off,
    LCD_LQ104V1DG28_SetBacklightLevel,
    LCD_LQ104V1DG28_GetBacklightLevel,
    LCD_LQ104V1DG28_SetPaletteColor,
    // v2.04
    LCD_LQ104V1DG28_WaitForVerticalSync,
};

const DEVICE_LCD LCD_SHARP_LQ104V1DG28_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:SHARP_LQ104V1DG28:I15Bit",
	    0x0100,
	    LCD_LQ104V1DG28_InitializeWorkspace_I15Bit,
	    sizeof(T_LQ104V1DG28Workspace),
	},
...
const DEVICE_LCD LCD_SHARP_LQ104V1DG28_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:SHARP_LQ104V1DG28:8Bit",
	    0x0100,
	    LCD_LQ104V1DG28_InitializeWorkspace_8Bit,
	    sizeof(T_LQ104V1DG28Workspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_SHARP_LQ104V1DG28_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_SHARP_LQ104V1DG28_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_SHARP_LQ104V1DG28_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_SHARP_LQ104V1DG28_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  Sharp_LQ104V1DG28.c
 *-------------------------------------------------------------------------*/
