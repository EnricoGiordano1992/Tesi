/*-------------------------------------------------------------------------*
 * File:  Sharp_LQ042T5DZ11.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the Sharp LQ042T5DZ11.
 *-------------------------------------------------------------------------*/
...
#ifndef LCD_SHARP_LQ042T5DZ11_FLIP_SCREEN_X
#define LCD_SHARP_LQ042T5DZ11_FLIP_SCREEN_X       0
#endif

#ifndef LCD_SHARP_LQ042T5DZ11_FLIP_SCREEN_Y
#define LCD_SHARP_LQ042T5DZ11_FLIP_SCREEN_Y       0
#endif
 ...
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_LQ042T5DZ11_settings;

static const T_LCDControllerSettings LCD_LQ042T5DZ11_params16bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_16_565,

    116,       /* Horizontal back porch */
    2,         /* Horizontal front porch */
    5,         /* HSYNC pulse width */
    480,       /* Pixels per line */

    31 - 2,    /* Vertical back porch (back porch - vertical pulse width(vsync))*/
...
static const T_LCDControllerSettings LCD_LQ042T5DZ11_paramsI15bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_LQ042T5DZ11_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_LQ042T5DZ11_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_LQ042T5DZ11_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_LQ042T5DZ11_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
#if LCD_SHARP_LQ042T5DZ11_FLIP_SCREEN_X
    #if LCD_SHARP_LQ042T5DZ11_FLIP_SCREEN_Y
        { 0x01, 0x610F },
    #else
        { 0x01, 0x630F },
    #endif
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_SHARP_LQ042T5DZ11_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LQ042T5DZ11 LCD.
 * Inputs:
 *      void *aW                    -- Particular workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_LQ042T5DZ11_InitializeWorkspace_16Bit(void *aW)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_LQ042T5DZ11_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LQ042T5DZ11 LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_LQ042T5DZ11_InitializeWorkspace_I15Bit(void *aW)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_LQ042T5DZ11_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LQ042T5DZ11 LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_LQ042T5DZ11_InitializeWorkspace_8Bit(void *aW)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_LQ042T5DZ11_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}
#if 0
static T_uezError SPIWriteCmd(
        T_LQ042T5DZ11Workspace *p,
...
#define LCD_SPI_ID1         (1<<2)
    ...
    r.iCSGPIOPort = p->iCSGPIOPort;
    r.iCSGPIOBit = p->iCSGPIOBit;
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_Open(void *aW)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aW;
        ...
        if (!error) {
            switch (p->iConfiguration->iColorDepth) {
                case UEZLCD_COLOR_DEPTH_8_BIT:
                    LCD_LQ042T5DZ11_settings = LCD_LQ042T5DZ11_params8bit;
                    break;
                default:
                case UEZLCD_COLOR_DEPTH_16_BIT:
                    LCD_LQ042T5DZ11_settings = LCD_LQ042T5DZ11_params16bit;
                    break;
                case UEZLCD_COLOR_DEPTH_I15_BIT:
                    LCD_LQ042T5DZ11_settings = LCD_LQ042T5DZ11_paramsI15bit;
                    break;
            }
            LCD_LQ042T5DZ11_settings.iBaseAddress = p->iBaseAddress;
            error = (*plcdc)->Configure(plcdc, &LCD_LQ042T5DZ11_settings);
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_Close(void *aW)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_LQ042T5DZ11_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aW;
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_On(void *aW)
{
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_Off(void *aW)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_LQ042T5DZ11_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_LQ042T5DZ11_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LQ042T5DZ11_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_LQ042T5DZ11Workspace *p = (T_LQ042T5DZ11Workspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_LQ042T5DZ11_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_SHARP_LQ042T5DZ11_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:SHARP_LQ042T5DZ11:16Bit",
	    0x0100,
	    LCD_LQ042T5DZ11_InitializeWorkspace_16Bit,
	    sizeof(T_LQ042T5DZ11Workspace),
	},
	
    // Functions
    LCD_LQ042T5DZ11_Open,
    LCD_LQ042T5DZ11_Close,
    LCD_LQ042T5DZ11_Configure,
    LCD_LQ042T5DZ11_GetInfo,
    LCD_LQ042T5DZ11_GetFrame,
    LCD_LQ042T5DZ11_ShowFrame,
    LCD_LQ042T5DZ11_On,
    LCD_LQ042T5DZ11_Off,
    LCD_LQ042T5DZ11_SetBacklightLevel,
    LCD_LQ042T5DZ11_GetBacklightLevel,
    LCD_LQ042T5DZ11_SetPaletteColor,
    // v2.04
    LCD_LQ042T5DZ11_WaitForVerticalSync,
};

const DEVICE_LCD LCD_SHARP_LQ042T5DZ11_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:SHARP_LQ042T5DZ11:I15Bit",
	    0x0100,
	    LCD_LQ042T5DZ11_InitializeWorkspace_I15Bit,
	    sizeof(T_LQ042T5DZ11Workspace),
	},
...
const DEVICE_LCD LCD_SHARP_LQ042T5DZ11_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:SHARP_LQ042T5DZ11:8Bit",
	    0x0100,
	    LCD_LQ042T5DZ11_InitializeWorkspace_8Bit,
	    sizeof(T_LQ042T5DZ11Workspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_SHARP_LQ042T5DZ11_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_SHARP_LQ042T5DZ11_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_SHARP_LQ042T5DZ11_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_SHARP_LQ042T5DZ11_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  Sharp_LQ042T5DZ11.c
 *-------------------------------------------------------------------------*/
