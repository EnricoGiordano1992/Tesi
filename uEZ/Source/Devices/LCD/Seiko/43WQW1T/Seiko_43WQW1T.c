/*-------------------------------------------------------------------------*
 * File:  Seiko_43WQW1T.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the SEIKO 43WQW1T LCD.
 *-------------------------------------------------------------------------*/

/*--------------------------------------------------------------------------
 * uEZ(R) - Copyright (C) 2007-2010 Future Designs, Inc.
 *--------------------------------------------------------------------------
 * This file is part of the uEZ(R) distribution.  See the included
 * uEZLicense.txt or visit http://www.teamfdi.com/uez for details.
 *
 *    *===============================================================*
 *    |  Future Designs, Inc. can port uEZ(tm) to your own hardware!  |
 *    |             We can get you up and running fast!               |
 *    |      See http://www.teamfdi.com/uez for more details.         |
 *    *===============================================================*
 *
 *-------------------------------------------------------------------------*/

#include <uEZ.h>
#include <HAL/LCDController.h>
#include <Device/Backlight.h>
#include <Types/SPI.h>
#include <Device/SPIBus.h>
#include <uEZDeviceTable.h>
#include "Seiko_43WQW1T.h"
#include <uEZGPIO.h>
#include <string.h>

/*---------------------------------------------------------------------------*
 * Constants:
 *---------------------------------------------------------------------------*/
#define RESOLUTION_X        480
#define RESOLUTION_Y        272
#define DOTCLK_HZ           8000000

/*---------------------------------------------------------------------------*
 * Types:
 *---------------------------------------------------------------------------*/

typedef struct {
	TUInt16 regAddress;
	TUInt16 dataByte;
} T_43WQW1TConfigData ;

typedef struct {
    const DEVICE_LCD *iHAL;
    TUInt32 iBaseAddress;
    int aNumOpen;
    TUInt32 iBacklightLevel;
    HAL_LCDController **iLCDController;
    DEVICE_SPI_BUS **iSPI;
    char iSPIBus[5];
    T_uezGPIOPortPin iCSGPIOPin;
    DEVICE_Backlight **iBacklight;
    const T_uezLCDConfiguration *iConfiguration;
    SPI_Request r;
    TUInt8 iCmd[3];
	T_uezSemaphore iVSyncSem;
} T_43WQW1TWorkspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static const T_43WQW1TConfigData G_LCDConfig[] = {
    {0x0003, 0xDB2C},
    {0x0010, 0x0600},
    {0x0011, 0x0501},
    {0x0012, 0x0007},
    {0x0013, 0x0500},
    {0x0014, 0x0305},
    {0x0015, 0x0606},
    {0x0016, 0x0703},
    {0x0017, 0x0400},
    {0x0018, 0x0F09},
    {0x0019, 0x0100},
    };

static T_LCDControllerSettings LCD_43WQW1TWorkspace_settings;

static const T_LCDControllerSettings LCD_43WQW1T_params16bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_16_565,

    2,         /* Horizontal back porch */
    2,         /* Horizontal front porch */
    41,         /* HSYNC pulse width */
    480,        /* Pixels per line */

    2,         /* Vertical back porch */
    1,         /* Vertical front porch */
    10,          /* VSYNC pulse width */
    272,        /* Lines per panel */

    0,          // Line end delay disabled

    EFalse,     /* Do not invert output enable */
    EFalse,     /* Invert panel clock */
    EFalse,     /* Invert HSYNC */
    EFalse,     /* Invert VSYNC */

    0,          /* AC bias frequency (not used) */

    EFalse,     // Not dual panel
    EFalse,     // Little endian (NOT big endian)
    EFalse,     //    Left to Right pixels (NOT right to left)
    EFalse,     // Top to bottom (NOT bottom to top)
    LCD_COLOR_ORDER_BGR,    // BGR order please

    0xA0000000, // Default Base address

    DOTCLK_HZ,    // 9 MHz is datasheet typical value (Max 15 MHz)
};

static T_LCDControllerSettings LCD_43WQW1T_paramsI15bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_16_I555,

    2,         /* Horizontal back porch */
    2,         /* Horizontal front porch */
    41,         /* HSYNC pulse width */
    480,        /* Pixels per line */

    2,         /* Vertical back porch */
    1,         /* Vertical front porch */
    10,          /* VSYNC pulse width */
    272,        /* Lines per panel */

    0,          // Line end delay disabled

    EFalse,     /* Do not invert output enable */
    EFalse,     /* Invert panel clock */
    EFalse,     /* Invert HSYNC */
    EFalse,     /* Invert VSYNC */

    0,          /* AC bias frequency (not used) */

    EFalse,     // Not dual panel
    EFalse,     // Little endian (NOT big endian)
    EFalse,     //    Left to Right pixels (NOT right to left)
    EFalse,     // Top to bottom (NOT bottom to top)
    LCD_COLOR_ORDER_BGR,    // RGB Color order

    0xA0000000, // Default Base address

    DOTCLK_HZ,    // 9 MHz is datasheet typical value (Max 15 MHz)
};

static const T_LCDControllerSettings LCD_43WQW1T_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,

    2,         /* Horizontal back porch */
    2,         /* Horizontal front porch */
    41,         /* HSYNC pulse width */
    480,        /* Pixels per line */

    2,         /* Vertical back porch */
    1,         /* Vertical front porch */
    10,          /* VSYNC pulse width */
    272,        /* Lines per panel */

    0,          // Line end delay disabled

    EFalse,     /* Do not invert output enable */
    EFalse,     /* Invert panel clock */
    EFalse,     /* Invert HSYNC */
    EFalse,     /* Invert VSYNC */

    0,          /* AC bias frequency (not used) */

    EFalse,     // Not dual panel
    EFalse,     // Little endian (NOT big endian)
    EFalse,     //    Left to Right pixels (NOT right to left)
    EFalse,     // Top to bottom (NOT bottom to top)
    LCD_COLOR_ORDER_BGR,    // BGR order please

    0xA0000000, // Default Base address

    DOTCLK_HZ,    // 9 MHz is datasheet typical value (Max 15 MHz)
};

static const T_uezLCDConfiguration LCD_43WQW1T_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
    UEZLCD_COLOR_DEPTH_16_BIT,
    UEZLCD_ORDER_RGB,
    0,
    1,
    RESOLUTION_X*2,
    RESOLUTION_X*2,
    256
};

static T_uezLCDConfiguration LCD_43WQW1T_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
    UEZLCD_COLOR_DEPTH_I15_BIT,
    UEZLCD_ORDER_RGB,
    0,
    1,
    RESOLUTION_X*2,
    RESOLUTION_X*2,
    256
};

static const T_uezLCDConfiguration LCD_43WQW1T_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
    UEZLCD_COLOR_DEPTH_8_BIT,
    UEZLCD_ORDER_RGB,
    0,
    1,
    RESOLUTION_X*1,
    RESOLUTION_X*1,
    256
};

/*---------------------------------------------------------------------------*
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_SEIKO_43WQW1T_Interface_16Bit;
extern const DEVICE_LCD LCD_SEIKO_43WQW1T_Interface_I15Bit;
extern const DEVICE_LCD LCD_SEIKO_43WQW1T_Interface_8Bit;
static void LCD_ConfigureSPI(T_43WQW1TWorkspace *p);

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for 43WQW1T LCD (16 bit mode).
 * Inputs:
 *      void *aW                    -- Particular SPI workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_InitializeWorkspace_16Bit(void *aW)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    p->iBaseAddress = 0x80000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_43WQW1T_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for 43WQW1T LCD (16 bit mode).
 * Inputs:
 *      void *aW                    -- Particular SPI workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_InitializeWorkspace_I15Bit(void *aW)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    p->iBaseAddress = 0x80000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_43WQW1T_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for 43WQW1T LCD.
 * Inputs:
 *      void *aW                    -- Particular SPI workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_InitializeWorkspace_8Bit(void *aW)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    p->iBaseAddress = 0x80000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_43WQW1T_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_Open
 *---------------------------------------------------------------------------*
 * Description:
 *      Start the LCD screen.  If this is the first open, initialize the
 *      screen.
 * Inputs:
 *      void *aW                -- Workspace
 * Outputs:
 *      T_uezError               -- If the device is opened, returns
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_Open(void *aW)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    HAL_LCDController **plcdc;
    T_uezError error = UEZ_ERROR_NONE;

    p->aNumOpen++;
    if (p->aNumOpen == 1) {
        plcdc = p->iLCDController;

        // Choose a settings based on the configuration
        switch (p->iConfiguration->iColorDepth) {
            case UEZLCD_COLOR_DEPTH_8_BIT:
                LCD_43WQW1TWorkspace_settings = LCD_43WQW1T_params8bit;
                break;
            default:
            case UEZLCD_COLOR_DEPTH_16_BIT:
                LCD_43WQW1TWorkspace_settings = LCD_43WQW1T_params16bit;
                break;
            case UEZLCD_COLOR_DEPTH_I15_BIT:
                LCD_43WQW1TWorkspace_settings = LCD_43WQW1T_paramsI15bit;
                break;
        }

        LCD_43WQW1TWorkspace_settings.iBaseAddress = p->iBaseAddress;

        // Turn on LCD controller and output power pin
        error = (*plcdc)->Configure(plcdc, &LCD_43WQW1TWorkspace_settings);

// Testing
(*p->iLCDController)->On(p->iLCDController);

        // Configure SPI
        LCD_ConfigureSPI(p);

//        return (*plcdc)->Configure(plcdc, &LCD_43WQW1T_settings);
        return error;
    }

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_Close
 *---------------------------------------------------------------------------*
 * Description:
 *      End access to the LCD display.
 * Inputs:
 *      void *aW                -- Workspace
 * Outputs:
 *      T_uezError               -- If the device is successfully closed,
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_Close(void *aW)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    p->aNumOpen--;

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_Configure
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LCD to use a particular LCD controller.
 * Inputs:
 *      void *aW                -- Workspace
 * Outputs:
 *      T_uezError               -- If the device is successfully configured,
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_Configure(
            void *aW,
            HAL_LCDController **aLCDController,
            TUInt32 aBaseAddress,
            DEVICE_Backlight **aBacklight)
{
    T_uezDevice spi;
    T_uezDeviceWorkspace *p_spi;
    
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    p->iLCDController = aLCDController;
    p->iBaseAddress = aBaseAddress;

    p->iBacklight = aBacklight;
   
    UEZDeviceTableFind(p->iSPIBus, &spi);
    UEZDeviceTableGetWorkspace(spi, (T_uezDeviceWorkspace **)&p_spi);

    p->iSPI = (DEVICE_SPI_BUS **)p_spi;

    p->r.iDataMOSI = p->iCmd;
    p->r.iDataMISO = p->iCmd;
    p->r.iNumTransfers = 0;
    p->r.iBitsPerTransfer = 8;
    p->r.iRate = 9000;
    p->r.iClockOutPolarity = ETrue;//EFalse;
    p->r.iClockOutPhase = EFalse;
    p->r.iCSGPIOPort = GPIO_TO_HAL_PORT(p->iCSGPIOPin);
    p->r.iCSGPIOBit = GPIO_TO_PIN_BIT(p->iCSGPIOPin);
    p->r.iCSPolarity = EFalse; // LOW true

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 *      Get information about the LCD device.
 * Inputs:
 *      void *aW                -- Workspace
 *      T_uezLCDConfiguration *aConfiguration -- Configuration settings
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_GetInfo(
                void *aW,
                T_uezLCDConfiguration *aConfiguration)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;

    // Use the setting based on color depth
    *aConfiguration = *p->iConfiguration;

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 *      Returns a pointer to the frame memory in the LCD display.
 * Inputs:
 *      void *aW                -- Workspace
 *      TUInt32 aFrame          -- Index to frame (0 based)
 *      void **aFrameBuffer     -- Pointer to base address
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    *aFrameBuffer = (void *)(p->iBaseAddress + aFrame * RESOLUTION_X * RESOLUTION_Y * 2);

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 *      Makes the passed frame the actively viewed frame on the LCD
 *      (if not already).
 * Inputs:
 *      void *aW                -- Workspace
 *      TUInt32 aFrame          -- Index to frame (0 based)
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    (*p->iLCDController)->SetBaseAddr(
            p->iLCDController,
            p->iBaseAddress + aFrame * RESOLUTION_X * RESOLUTION_Y * 2,
            EFalse);

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_On
 *---------------------------------------------------------------------------*
 * Description:
 *      Turns on the LCD display.
 * Inputs:
 *      void *aW                -- Workspace
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_On(void *aW)
{
    // Turn back on to the remembered level
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
    (*p->iLCDController)->On(p->iLCDController);
    (*p->iBacklight)->On(p->iBacklight);

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_Off
 *---------------------------------------------------------------------------*
 * Description:
 *      Turns off the LCD display.
 * Inputs:
 *      void *aW                -- Workspace
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_Off(void *aW)
{
    // Turn off, but don't remember the level
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;
//TESTING    (*p->iLCDController)->Off(p->iLCDController);
    (*p->iBacklight)->Off(p->iBacklight);

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_Backlight
 *---------------------------------------------------------------------------*
 * Description:
 *      Turns on or off the backlight.  A value of 0 is off and values of 1
 *      or higher is higher levels of brightness (dependent on the LCD
 *      display).  If a display is on/off only, this value will be 1 or 0
 *      respectively.
 * Inputs:
 *      void *aW                -- Workspace
 *      TUInt32 aLevel          -- Level of backlight
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *                                  If the backlight intensity level is
 *                                  invalid, returns UEZ_ERROR_OUT_OF_RANGE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_Backlight(void *aW, TUInt32 aLevel)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;

    // Limit the backlight level
    if (aLevel > p->iConfiguration->iNumBacklightLevels)
        aLevel = p->iConfiguration->iNumBacklightLevels;

    // Remember this level and use it
    p->iBacklightLevel = aLevel;

    return (*p->iBacklight)->SetRatio(p->iBacklight,
        (aLevel * 0xFFFF) / (p->iConfiguration->iNumBacklightLevels));
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 *      Turns on or off the backlight.  A value of 0 is off and values of 1
 *      or higher is higher levels of brightness (dependent on the LCD
 *      display).  If a display is on/off only, this value will be 1 or 0
 *      respectively.
 * Inputs:
 *      void *aW                -- Workspace
 *      TUInt32 aLevel          -- Level of backlight
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *                                  If the backlight intensity level is
 *                                  invalid, returns UEZ_ERROR_OUT_OF_RANGE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;

    // Limit the backlight level
    if (aLevel > p->iConfiguration->iNumBacklightLevels)
        aLevel = p->iConfiguration->iNumBacklightLevels;

    // Remember this level and use it
    p->iBacklightLevel = aLevel;

    return (*p->iBacklight)->SetRatio(p->iBacklight,
        (aLevel * 0xFFFF) / (p->iConfiguration->iNumBacklightLevels));
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 *      Returns the current backlight level as well as the number of
 *      maximum light levels available.
 * Inputs:
 *      void *aW                -- Workspace
 *      TUInt32 *aLevel          -- Level of backlight returned
 *      TUInt32 *aNumLevels      -- Number of levels maximum
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *                                  If the backlight intensity level is
 *                                  invalid, returns UEZ_ERROR_OUT_OF_RANGE.
 *                                  Returns UEZ_ERROR_NOT_SUPPORTED if
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_43WQW1T_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aW;

    if (!p->iBacklight)
        return UEZ_ERROR_NOT_SUPPORTED;

    if (aNumLevels)
        *aNumLevels = p->iConfiguration->iNumBacklightLevels;

    // Remember this level and use it
    *aLevel = p->iBacklightLevel;

    return UEZ_ERROR_NONE;
}



/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 *      Change the color of a palette entry given the red, green, blue
 *      component of a particular color index.  The color components are
 *      expressed in full 16-bit values at a higher resolution than
 *      the hardware can usually perform.  The color is down shifted to what
 *      the hardware can handle.
 * Inputs:
 *      void *aW                    -- Workspace
 *      TUInt32 aColorIndex         -- Index to palette entry
 *      TUInt16 aRed                -- Red value
 *      TUInt16 aGreen              -- Green value
 *      TUInt16 aBlue               -- Blue value
 * Outputs:
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *                                  If the index is invalid, returns
 *                                  UEZ_ERROR_OUT_OF_RANGE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_43WQW1T_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aWorkspace;

    return (*p->iLCDController)->SetPaletteColor(
                p->iLCDController,
                aColorIndex,
                aRed,
                aGreen,
                aBlue);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_ConfigureSPI

 *---------------------------------------------------------------------------*
 * Description:
 *      Sends configuration data to the LCD via the SPI Bus 
 * Inputs:
 *      void *aW                -- Workspace
 * Outputs:
 *      NONE
 *---------------------------------------------------------------------------*/
static void LCD_ConfigureSPI(T_43WQW1TWorkspace *p)
{
     TUInt8 i;
     
     for (i=0; i<10; i++)
     {
        p->iCmd[0] = 0x70; // command byte to send register
        p->iCmd[1] = (G_LCDConfig[i].regAddress>>8);
        p->iCmd[2] = (G_LCDConfig[i].regAddress &0xFF);
        p->r.iNumTransfers = 3;

        (*p->iSPI)->TransferPolled(p->iSPI, &p->r);
        
        p->iCmd[0] = 0x72; // command byte to send register
        p->iCmd[1] = (G_LCDConfig[i].dataByte>>8);
        p->iCmd[2] = (G_LCDConfig[i].dataByte &0xFF);
        p->r.iNumTransfers = 3;

        (*p->iSPI)->TransferPolled(p->iSPI, &p->r);
     }
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 *      This routine is called from within an interrupt when the vertical
 *      sync occurs.  This routines releases any waiting callers on
 *      the semaphore.
 * Inputs:
 *      void *aCallbackWorkspace -- LCD workspace
 *---------------------------------------------------------------------------*/
static void LCD_43WQW1T_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_43WQW1T_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 *      Wait for a vertical sync be enabling the vertical sync callback
 *      and waiting on a semaphore.
 * Inputs:
 *      void *aWorkspace -- LCD workspace
 *      TUInt32 aTimeout -- Timeout in ms or UEZ_TIMEOUT_INFINITE
 * Outputs:
 *      T_uezError -- UEZ_ERROR_TIMEOUT or UEZ_ERROR_OK.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_43WQW1T_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_43WQW1TWorkspace *p = (T_43WQW1TWorkspace *)aWorkspace;
    HAL_LCDController **p_hal = p->iLCDController;

    UEZSemaphoreGrab(p->iVSyncSem, 0);
    (*p_hal)->EnableVerticalSync(p_hal, LCD_43WQW1T_VerticalSyncCallback, p);
    return UEZSemaphoreGrab(p->iVSyncSem, aTimeout);
}

void LCD_43WQW1T_Create(char* aName,
                        char* aSPIBus,
                        T_uezGPIOPortPin aSPICSPin)
{
    T_43WQW1TWorkspace *p;

    T_uezDevice lcd;
    T_uezDeviceWorkspace *p_lcd;

    UEZDeviceTableFind(aName, &lcd);
    UEZDeviceTableGetWorkspace(lcd, &p_lcd);

    p = (T_43WQW1TWorkspace *)p_lcd;

    strcpy(p->iSPIBus, aSPIBus);
    p->iCSGPIOPin = aSPICSPin;
}

/*---------------------------------------------------------------------------*
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_SEIKO_43WQW1T_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:SEIKO_43WQW1T:16Bit",
	    0x0100,
	    LCD_43WQW1T_InitializeWorkspace_16Bit,
	    sizeof(T_43WQW1TWorkspace),
	},
	
    // Functions
    LCD_43WQW1T_Open,
    LCD_43WQW1T_Close,
    LCD_43WQW1T_Configure,
    LCD_43WQW1T_GetInfo,
    LCD_43WQW1T_GetFrame,
    LCD_43WQW1T_ShowFrame,
    LCD_43WQW1T_On,
    LCD_43WQW1T_Off,
    LCD_43WQW1T_SetBacklightLevel,
    LCD_43WQW1T_GetBacklightLevel,
    LCD_43WQW1T_SetPaletteColor,
    // v2.04
    LCD_43WQW1T_WaitForVerticalSync,
};

const DEVICE_LCD LCD_SEIKO_43WQW1T_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:SEIKO_43WQW1T:I15Bit",
	    0x0100,
	    LCD_43WQW1T_InitializeWorkspace_I15Bit,
	    sizeof(T_43WQW1TWorkspace),
	},
	
    // Functions
    LCD_43WQW1T_Open,
    LCD_43WQW1T_Close,
    LCD_43WQW1T_Configure,
    LCD_43WQW1T_GetInfo,
    LCD_43WQW1T_GetFrame,
    LCD_43WQW1T_ShowFrame,
    LCD_43WQW1T_On,
    LCD_43WQW1T_Off,
    LCD_43WQW1T_SetBacklightLevel,
    LCD_43WQW1T_GetBacklightLevel,
    LCD_43WQW1T_SetPaletteColor,
    // v2.04
    LCD_43WQW1T_WaitForVerticalSync,
};

const DEVICE_LCD LCD_SEIKO_43WQW1T_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:SEIKO_43WQW1T:8Bit",
	    0x0100,
	    LCD_43WQW1T_InitializeWorkspace_8Bit,
	    sizeof(T_43WQW1TWorkspace),
	},
	
    // Functions
    LCD_43WQW1T_Open,
    LCD_43WQW1T_Close,
    LCD_43WQW1T_Configure,
    LCD_43WQW1T_GetInfo,
    LCD_43WQW1T_GetFrame,
    LCD_43WQW1T_ShowFrame,
    LCD_43WQW1T_On,
    LCD_43WQW1T_Off,
    LCD_43WQW1T_SetBacklightLevel,
    LCD_43WQW1T_GetBacklightLevel,
    LCD_43WQW1T_SetPaletteColor,
    // v2.04
    LCD_43WQW1T_WaitForVerticalSync,
};

// List of interfaces
const T_uezDeviceInterface *LCD_SEIKO_43WQW1T_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_SEIKO_43WQW1T_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_SEIKO_43WQW1T_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_SEIKO_43WQW1T_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  Seiko_43WQW1T.c
 *-------------------------------------------------------------------------*/
