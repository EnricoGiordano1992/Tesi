/*-------------------------------------------------------------------------*
 * File:  Hitachi_TX11D06VM2APA.c
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the Hitachi_TX11D06VM2APA.
 *-------------------------------------------------------------------------*/
...
#ifndef LCD_Hitachi_TX11D06VM2APA_FLIP_SCREEN_X
#define LCD_Hitachi_TX11D06VM2APA_FLIP_SCREEN_X       0
#endif

#ifndef LCD_Hitachi_TX11D06VM2APA_FLIP_SCREEN_Y
#define LCD_Hitachi_TX11D06VM2APA_FLIP_SCREEN_Y       0
#endif
 ...
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_TX11D06VM2APA_settings;

static const T_LCDControllerSettings LCD_TX11D06VM2APA_params16bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_TX11D06VM2APA_paramsI15bit = {
    LCD_ADVANCED_TFT,
...
static const T_LCDControllerSettings LCD_TX11D06VM2APA_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_TX11D06VM2APA_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_TX11D06VM2APA_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_TX11D06VM2APA_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_Hitachi_TX11D06VM2APA_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TX11D06VM2APA LCD.
 * Inputs:
 *      void *aW                    -- Particular workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TX11D06VM2APA_InitializeWorkspace_16Bit(void *aW)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TX11D06VM2APA_configuration_16Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TX11D06VM2APA LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TX11D06VM2APA_InitializeWorkspace_I15Bit(void *aW)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TX11D06VM2APA_configuration_I15Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for TX11D06VM2APA LCD.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LCD_TX11D06VM2APA_InitializeWorkspace_8Bit(void *aW)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aW;
    p->iBaseAddress = 0xA0000000;
    p->aNumOpen = 0;
    p->iBacklightLevel = 256; // 100%
    p->iConfiguration = &LCD_TX11D06VM2APA_configuration_8Bit;

    return UEZSemaphoreCreateBinary(&p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_Open
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_Open(void *aW)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aW;
        ...
        if (!error) {
            switch (p->iConfiguration->iColorDepth) {
                case UEZLCD_COLOR_DEPTH_8_BIT:
                    LCD_TX11D06VM2APA_settings = LCD_TX11D06VM2APA_params8bit;
                    break;
                default:
                case UEZLCD_COLOR_DEPTH_16_BIT:
                    LCD_TX11D06VM2APA_settings = LCD_TX11D06VM2APA_params16bit;
                    break;
                case UEZLCD_COLOR_DEPTH_I15_BIT:
                    LCD_TX11D06VM2APA_settings = LCD_TX11D06VM2APA_paramsI15bit;
                    break;
            }
            LCD_TX11D06VM2APA_settings.iBaseAddress = p->iBaseAddress;
            error = (*plcdc)->Configure(plcdc, &LCD_TX11D06VM2APA_settings);
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_Close
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_Close(void *aW)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_TX11D06VM2APA_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aW;
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_On(void *aW)
{
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_Off(void *aW)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_VerticalSyncCallback
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void LCD_TX11D06VM2APA_VerticalSyncCallback(void *aCallbackWorkspace)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aCallbackWorkspace;

    _isr_UEZSemaphoreRelease(p->iVSyncSem);
}

/*---------------------------------------------------------------------------*
 * Routine:  LCD_TX11D06VM2APA_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_TX11D06VM2APA_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_TX11D06VM2APAWorkspace *p = (T_TX11D06VM2APAWorkspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_TX11D06VM2APA_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_Hitachi_TX11D06VM2APA_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:Hitachi_TX11D06VM2APA:16Bit",
	    0x0100,
	    LCD_TX11D06VM2APA_InitializeWorkspace_16Bit,
	    sizeof(T_TX11D06VM2APAWorkspace),
	},
	
    // Functions
    LCD_TX11D06VM2APA_Open,
    LCD_TX11D06VM2APA_Close,
    LCD_TX11D06VM2APA_Configure,
    LCD_TX11D06VM2APA_GetInfo,
    LCD_TX11D06VM2APA_GetFrame,
    LCD_TX11D06VM2APA_ShowFrame,
    LCD_TX11D06VM2APA_On,
    LCD_TX11D06VM2APA_Off,
    LCD_TX11D06VM2APA_SetBacklightLevel,
    LCD_TX11D06VM2APA_GetBacklightLevel,
    LCD_TX11D06VM2APA_SetPaletteColor,
    // v2.04
    LCD_TX11D06VM2APA_WaitForVerticalSync,
};

const DEVICE_LCD LCD_Hitachi_TX11D06VM2APA_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:Hitachi_TX11D06VM2APA:I15Bit",
	    0x0100,
	    LCD_TX11D06VM2APA_InitializeWorkspace_I15Bit,
	    sizeof(T_TX11D06VM2APAWorkspace),
	},
...
const DEVICE_LCD LCD_Hitachi_TX11D06VM2APA_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:Hitachi_TX11D06VM2APA:8Bit",
	    0x0100,
	    LCD_TX11D06VM2APA_InitializeWorkspace_8Bit,
	    sizeof(T_TX11D06VM2APAWorkspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_Hitachi_TX11D06VM2APA_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_Hitachi_TX11D06VM2APA_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_Hitachi_TX11D06VM2APA_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_Hitachi_TX11D06VM2APA_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};

/*-------------------------------------------------------------------------*
 * End of File:  Hitachi_TX11D06VM2APA.c
 *-------------------------------------------------------------------------*/
