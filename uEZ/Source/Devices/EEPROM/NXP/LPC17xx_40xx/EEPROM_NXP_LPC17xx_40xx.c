/*-------------------------------------------------------------------------*
 * File:  EEPROM_NXP_LPC17xx_40xx_driver.c
 *-------------------------------------------------------------------------*
 * Description:
 *      Device implementation of the EEPROM for the LPC17xx_40xx
 *-------------------------------------------------------------------------*/
...
#include "EEPROM_NXP_LPC17xx_40xx.h"

/*---------------------------------------------------------------------------*
 * Constants:
 *---------------------------------------------------------------------------*/
#define LPC17xx_40xx_EEPROM_PAGE_SIZE    64 // bytes
...
static T_EEPROM_NXP_LPC17xx_40xx_Workspace *G_lpc1788EEPROMWorkspace;
...
static void IProcessIRQ(T_EEPROM_NXP_LPC17xx_40xx_Workspace *p)
{
...
static void IReadPage(
        T_EEPROM_NXP_LPC17xx_40xx_Workspace *p, 
        TUInt32 aAddress, 
        TUInt8 *aBuffer, 
        TUInt8 aSize)
{
...
//    p->end_of_rdwr = 0;
    ...
    if (aSize == 0) {
        ...
        for (i = 0; i < LPC17xx_40xx_EEPROM_PAGE_SIZE; i++) {
        ...
        for (i = 0; i < (LPC17xx_40xx_EEPROM_PAGE_SIZE/4); i++) {
            *((TUInt32 *)(aBuffer + (i * 4))) = LPC_EEPROM->RDATA;
        }
    } else {
        // Read 16-bit words
        LPC_EEPROM->CMD = RD_PFETCH|READ16; 
        for (i = 0; i < (LPC17xx_40xx_EEPROM_PAGE_SIZE/2); i++) {
            data = LPC_EEPROM->RDATA;
...
/*---------------------------------------------------------------------------*
 * Routine:  EEPROM_NXP_LPC17xx_40xx_Read
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError EEPROM_NXP_LPC17xx_40xx_Read(
        void *aWorkspace,
        TUInt32 aAddress,
        TUInt8 *aBuffer,
        TUInt32 aNumBytes)
{
    ...
    while (size) {
        ...
        // Read in a page
        IReadPage(p, aAddress&(~(LPC17xx_40xx_EEPROM_PAGE_SIZE-1)), p->iPage, 0);
        
        // Copy over the part we need (offset by the address)
        memcpy(aBuffer, p->iPage+(aAddress & (LPC17xx_40xx_EEPROM_PAGE_SIZE-1)), len);
...
T_uezError EEPROM_NXP_LPC17xx_40xx_SetConfiguration(
        void *aWorkspace,
 ...
 *---------------------------------------------------------------------------*/
const DEVICE_EEPROM EEPROM_NXP_LPC17xx_40xx_Interface = {
	{
		// Common device interface
	    "EEPROM:NXP:LPC17xx_40xx",
	    0x0100,
	    EEPROM_NXP_LPC17xx_40xx_InitializeWorkspace,
	    sizeof(T_EEPROM_NXP_LPC17xx_40xx_Workspace),
	},
	
    // Functions
    EEPROM_NXP_LPC17xx_40xx_SetConfiguration,
    EEPROM_NXP_LPC17xx_40xx_Read,
    EEPROM_NXP_LPC17xx_40xx_Write,
} ;

/*-------------------------------------------------------------------------*
 * End of File:  EEPROM_NXP_LPC17xx_40xx_driver.c
 *-------------------------------------------------------------------------*/
