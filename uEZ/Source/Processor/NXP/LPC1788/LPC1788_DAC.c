 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1788 DAC Interface.
 * Implementation:
...
typedef struct {
    ...
    TUInt32 iVRef;
    TBool iBias;
} T_LPC1788_DAC_Workspace;

/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_DAC_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 DAC workspace.
 * Inputs:
 *      void *aW                    -- Particular DAC workspace
 * Outputs:
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_DAC_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1788_DAC_Workspace *p = (T_LPC1788_DAC_Workspace *)aWorkspace;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_DAC_Write(void *aWorkspace, TUInt32 aValue)
{
    T_LPC1788_DAC_Workspace *p = (T_LPC1788_DAC_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_DAC_WriteMilliVolt
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_DAC_WriteMilliVolt(void *aWorkspace, TUInt32 aMilliVolts)
{
    T_LPC1788_DAC_Workspace *p = (T_LPC1788_DAC_Workspace *)aWorkspace;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_DAC_Read(void *aWorkspace, TUInt32 *aValue)
{
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_DAC_ReadMilliVolt(void *aWorkspace, TUInt32 * aMilliVolts)
{
    TUInt32 value;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_DAC_SetBias(void *aWorkspace, TBool aBias)
{
    T_LPC1788_DAC_Workspace *p = (T_LPC1788_DAC_Workspace *)aWorkspace;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_DAC_SetVRef(void *aWorkspace, const TUInt32 aVRef)
{
    T_LPC1788_DAC_Workspace *p = (T_LPC1788_DAC_Workspace *)aWorkspace;
 ...
 *---------------------------------------------------------------------------*/
const HAL_DAC DAC_LPC1788_DAC0_Interface = {
        {
                "LPC1788:DAC0",
                0x0100,
                LPC1788_DAC_InitializeWorkspace,
                sizeof(T_LPC1788_DAC_Workspace), },

        LPC1788_DAC_Read,
        LPC1788_DAC_ReadMilliVolt,
        LPC1788_DAC_Write,
        LPC1788_DAC_WriteMilliVolt,
        LPC1788_DAC_SetBias,
        LPC1788_DAC_SetVRef };
 ...
 *---------------------------------------------------------------------------*/
void LPC1788_DAC0_Require(T_uezGPIOPortPin aPin)
{
    static const T_LPC1788_IOCON_ConfigList dac0[] = {
    ...
    // Register DAC0
    HALInterfaceRegister("DAC0", (T_halInterface *)&DAC_LPC1788_DAC0_Interface,
            0, 0);
    LPC1788_IOCON_ConfigPin(aPin, dac0, ARRAY_COUNT(dac0));
}
