...
#include <HAL/GPDMA.h>
#include "LPC1788_MCI.h"
#include "LPC1788_GPIO.h"
...
T_LPC1788_MCI_Workspace *G_mciWorkspace;
 ...
 * Prototypes:
 *-------------------------------------------------------------------------*/
void LPC1788_MCI_ProcessInterrupt(T_LPC1788_MCI_Workspace *p);

/*---------------------------------------------------------------------------*
 * Interrupt:  LPC1788_MCI_Interrupt
 *---------------------------------------------------------------------------*
 * Description:
 *      A MCI Interrupt has come in.  Process the one workspace attached
 *      to this interrupt.
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE(LPC1788_MCI_Interrupt)
{
    IRQ_START();
    LPC1788_MCI_ProcessInterrupt(G_mciWorkspace);
    IRQ_END();
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_ProcessInterrupt
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      void
 *---------------------------------------------------------------------------*/
void LPC1788_MCI_ProcessInterrupt(
    T_LPC1788_MCI_Workspace *p)
{
    TUInt32 ms;
    TUInt8 xs;
    //TUInt32 n;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_InitializeWorkspace(void *aWorkspace)
{
    TUInt32 n;
    TUInt32 dma_control;
    T_LPC1788_MCI_Workspace *p = (T_LPC1788_MCI_Workspace *)aWorkspace;
    p->iHAL = &LPC1788_MCI_Interface;
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_PowerOn(void *aWorkspace)
{
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_PowerOff(void *aWorkspace)
{
    MCI_MASK0 = 0;
    MCI_COMMAND = 0;
    MCI_DATA_CTRL = 0;
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_Reset(void *aWorkspace)
{
    // Reset MCI Registers
    LPC_MCI->MASK0 = 0;
    LPC_MCI->COMMAND = 0;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_SendCommand
 *---------------------------------------------------------------------------*
 * Description:
    ...
    TUInt32 *aResponse)
{
    ...
    if (aCommand & 0x80) {
        ...
        if (!LPC1788_MCI_SendCommand(aWorkspace, MCI_CMD55, (TUInt32)p->iRCA << 16UL,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_ReadyReception
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_ReadyReception(
    void *aWorkspace,
    TUInt32 aNumBlocks,
    TUInt32 aBlockSize,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_ReadyTransmission
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_ReadyTransmission(
    void *aWorkspace,
    TUInt32 aNumBlocks,
    TUInt32 aBlockSize,
    TUInt32 *aNumWriting)
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_StartTransmission
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_StartTransmission(
    void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_StopTransfer
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_StopTransfer(void *aWorkspace)
{
    T_LPC1788_MCI_Workspace *p = (T_LPC1788_MCI_Workspace *)aWorkspace;
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_Read(void *aWorkspace, void *aBuffer, TUInt32 aReadSize)
{
    T_LPC1788_MCI_Workspace *p = (T_LPC1788_MCI_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_SetRelativeCardAddress
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_SetRelativeCardAddress(void *aWorkspace, TUInt32 aRCA)
{
    T_LPC1788_MCI_Workspace *p = (T_LPC1788_MCI_Workspace *)aWorkspace;
    p->iRCA = aRCA;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_SetClockRate
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_SetClockRate(
    void *aWorkspace,
    TUInt32 aHz,
    TBool aPowerSave)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_MCI_SetDataBus
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError              -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_MCI_SetDataBus(void *aWorkspace, T_uezMCIBus aBus)
{
    switch (aBus) {
...
TBool LPC1788_MCI_IsWriteAvailable(void *aWorkspace)
{
    T_LPC1788_MCI_Workspace *p = (T_LPC1788_MCI_Workspace *)aWorkspace;
...
TBool LPC1788_MCI_IsWriteEmpty(void *aWorkspace)
{
    T_LPC1788_MCI_Workspace *p = (T_LPC1788_MCI_Workspace *)aWorkspace;
 ...
 *---------------------------------------------------------------------------*/
const HAL_MCI LPC1788_MCI_Interface = { {
    "LPC1788:MCI",
    0x0204,
    LPC1788_MCI_InitializeWorkspace,
    sizeof(T_LPC1788_MCI_Workspace), },

// v2.04
    LPC1788_MCI_PowerOn,
    LPC1788_MCI_PowerOff,
    LPC1788_MCI_Reset,
    LPC1788_MCI_SendCommand,
    LPC1788_MCI_ReadyReception,
    LPC1788_MCI_StopTransfer,
    LPC1788_MCI_Read,
    LPC1788_MCI_SetRelativeCardAddress,
    LPC1788_MCI_SetClockRate,
    LPC1788_MCI_SetDataBus,
    LPC1788_MCI_ReadyTransmission,
    LPC1788_MCI_StartTransmission,
    LPC1788_MCI_Write,
    LPC1788_MCI_IsWriteAvailable,
    LPC1788_MCI_IsWriteEmpty,
};

/*---------------------------------------------------------------------------*
 * Requirement routines:
 *---------------------------------------------------------------------------*/
void LPC1788_MCI_Require(const T_LPC1788_MCI_Pins *aPins, const char *aGPDMA)
{
    T_halWorkspace *p_mci;
    T_LPC1788_MCI_Workspace *p;

    static const T_LPC1788_IOCON_ConfigList pinsDAT0[] = {
            {GPIO_P0_22, IOCON_D_DEFAULT(2)},
            {GPIO_P1_6,  IOCON_D_DEFAULT(2)},
    };
    static const T_LPC1788_IOCON_ConfigList pinsDAT1[] = {
            {GPIO_P1_7,  IOCON_D_DEFAULT(2)},
            {GPIO_P2_11, IOCON_D_DEFAULT(2)},
    };
    static const T_LPC1788_IOCON_ConfigList pinsDAT2[] = {
            {GPIO_P1_11, IOCON_D_DEFAULT(2)},
            {GPIO_P2_12, IOCON_D_DEFAULT(2)},
    };
    static const T_LPC1788_IOCON_ConfigList pinsDAT3[] = {
            {GPIO_P1_12, IOCON_D_DEFAULT(2)},
            {GPIO_P2_13, IOCON_D_DEFAULT(2)},
    };
    static const T_LPC1788_IOCON_ConfigList pinsCLK[] = {
            {GPIO_P0_19, IOCON_D_DEFAULT(2)},
            {GPIO_P1_2,  IOCON_D_DEFAULT(2)},
    };
    static const T_LPC1788_IOCON_ConfigList pinsCMD[] = {
    ...
    HALInterfaceRegister("MCI",
            (T_halInterface *)&LPC1788_MCI_Interface, 0, &p_mci);
    p = (T_LPC1788_MCI_Workspace *)p_mci;
    ...
    LPC1788_IOCON_ConfigPin(aPins->iDAT0, pinsDAT0, ARRAY_COUNT(pinsDAT0));
    LPC1788_IOCON_ConfigPin(aPins->iDAT1, pinsDAT1, ARRAY_COUNT(pinsDAT1));
    LPC1788_IOCON_ConfigPin(aPins->iDAT2, pinsDAT2, ARRAY_COUNT(pinsDAT2));
    LPC1788_IOCON_ConfigPin(aPins->iDAT3, pinsDAT3, ARRAY_COUNT(pinsDAT3));
    LPC1788_IOCON_ConfigPin(aPins->iCLK, pinsCLK, ARRAY_COUNT(pinsCLK));
    LPC1788_IOCON_ConfigPin(aPins->iCMD, pinsCMD, ARRAY_COUNT(pinsCMD));
    ...
    InterruptRegister(MCI_IRQn, LPC1788_MCI_Interrupt,
