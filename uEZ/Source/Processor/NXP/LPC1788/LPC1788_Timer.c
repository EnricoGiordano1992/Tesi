 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1788 Timers Interface.
 * Implementation:
...
} T_LPC1788_Timer_Info;

typedef struct {
    T_HALTimer_Callback iCallbackFunc;
    void *iCallbackWorkspace;
} T_LPC1788_Timer_InterruptCallback;

typedef struct {
    const HAL_Timer *iHAL;
    T_LPC1788_Timer_Registers *iReg;
    T_LPC1788_Timer_InterruptCallback iCallbacks[4]; // one per match register
    const T_LPC1788_Timer_Info *iInfo;
} T_LPC1788_Timer_Workspace;

/*---------------------------------------------------------------------------*
 * Prototypes:
 *---------------------------------------------------------------------------*/
static const T_LPC1788_Timer_Info G_Timer0_Info;
static const T_LPC1788_Timer_Info G_Timer1_Info;
static const T_LPC1788_Timer_Info G_Timer2_Info;
static const T_LPC1788_Timer_Info G_Timer3_Info;
T_LPC1788_Timer_Workspace *G_Timer0_Workspace;
T_LPC1788_Timer_Workspace *G_Timer1_Workspace;
T_LPC1788_Timer_Workspace *G_Timer2_Workspace;
T_LPC1788_Timer_Workspace *G_Timer3_Workspace;
static void ITimerProcessIRQ(T_LPC1788_Timer_Workspace *p);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer_SetTimerMode
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_Timer_SetTimerMode(
        void *aWorkspace,
        T_Timer_Mode aMode)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    T_LPC1788_Timer_Registers *r = p->iReg;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer_SetCaptureSource
 *---------------------------------------------------------------------------*
 * Description:
 *      Timer is going to start -- chip select enabled for device
 * Inputs:
 ...
 *                                  does not exist.
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_Timer_SetCaptureSource(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer_SetMatchRegister
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  when match occurs.
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_Timer_SetMatchRegister(
        void *aWorkspace,
        ...
        TBool aDoStop)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    T_LPC1788_Timer_Registers *r = p->iReg;
    const T_LPC1788_Timer_Info *p_info = p->iInfo;
    TUInt32 aFlags = 0;

    //Make sure Power to the Timer is turned on
    LPC1788PowerOn(1 << p_info->iPCONBitIndex);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer_SetExternalControl
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_Timer_SetExternalControl(
        void *aWorkspace,
        TUInt8 aMatchRegister,
 ...
 *      void *aWorkspace        -- Timer's workspace
 *---------------------------------------------------------------------------*/
static void LPC1788_Timer_Reset(void *aWorkspace)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    T_LPC1788_Timer_Registers *r = p->iReg;
    int i;
 ...
 *      void *aWorkspace        -- Timer's workspace
 *---------------------------------------------------------------------------*/
static void LPC1788_Timer_Enable(void *aWorkspace)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    T_LPC1788_Timer_Registers *r = p->iReg;
 ...
 *      void *aWorkspace        -- Timer's workspace
 *---------------------------------------------------------------------------*/
static void LPC1788_Timer_Disable(void *aWorkspace)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    T_LPC1788_Timer_Registers *r = p->iReg;
 ...
 *          of range.
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_Timer_SetMatchCallback(
        void *aWorkspace,
        TUInt8 aMatchRegister,
 ...
 *---------------------------------------------------------------------------*/
static void IClear(T_LPC1788_Timer_InterruptCallback aCallbacks[4])
{
    TUInt8 i;
    for (i = 0; i < 4; i++) {
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer0_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 Timer0 workspace.
 * Inputs:
 *      void *aW                    -- Particular Timer workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_Timer0_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    p->iReg = (T_LPC1788_Timer_Registers *)LPC_TIM0_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer1_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 Timer1 workspace.
 * Inputs:
 *      void *aW                    -- Particular Timer workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_Timer1_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    p->iReg = (T_LPC1788_Timer_Registers *)LPC_TIM1_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer2_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 Timer1 workspace.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_Timer2_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    p->iReg = (T_LPC1788_Timer_Registers *)LPC_TIM2_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer3_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 Timer1 workspace.
 * Inputs:
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_Timer3_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1788_Timer_Workspace *p = (T_LPC1788_Timer_Workspace *)aWorkspace;
    p->iReg = (T_LPC1788_Timer_Registers *)LPC_TIM3_BASE;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_Timer_SetMatchRegisterFunctions
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *          UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_Timer_SetMatchRegisterFunctions(
        void *aWorkspace,
        TUInt8 aMatchRegister,
        ...
        TUInt32 aMatchPoint)
{
    ...
    //Make sure Power to the Timer is turned on
    LPC1788PowerOn(1UL << p_info->iPCONBitIndex);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_Timer LPC1788_Timer0_Interface = {
    {
    "NXP:LPC1788:Timer0",
    0x0100,
    LPC1788_Timer0_InitializeWorkspace,
    sizeof(T_LPC1788_Timer_Workspace),
    },

    LPC1788_Timer_SetTimerMode,
    LPC1788_Timer_SetCaptureSource,
    LPC1788_Timer_SetMatchRegister,
    LPC1788_Timer_SetExternalControl,
    LPC1788_Timer_Reset,
    LPC1788_Timer_Enable,
    LPC1788_Timer_Disable,
    LPC1788_Timer_SetMatchCallback,
    LPC1788_Timer_SetMatchRegisterFunctions,
};

const HAL_Timer LPC1788_Timer1_Interface = {
    {
    "NXP:LPC1788:Timer1",
    0x0100,
    LPC1788_Timer1_InitializeWorkspace,
    sizeof(T_LPC1788_Timer_Workspace),
    },
...
const HAL_Timer LPC1788_Timer2_Interface = {
    {
    "NXP:LPC1788:Timer2",
    0x0100,
    LPC1788_Timer2_InitializeWorkspace,
    sizeof(T_LPC1788_Timer_Workspace),
    },
...
const HAL_Timer LPC1788_Timer3_Interface = {
    {
    "NXP:LPC1788:Timer3",
    0x0100,
    LPC1788_Timer3_InitializeWorkspace,
    sizeof(T_LPC1788_Timer_Workspace),
    },
...
{
    static const T_LPC1788_IOCON_ConfigList pinsCAP0[] = {
            {GPIO_P1_26, IOCON_D_DEFAULT(3)}, // T0_CAP0
            {GPIO_P3_23, IOCON_D_DEFAULT(3)}, // T0_CAP0
    };
    static const T_LPC1788_IOCON_ConfigList pinsCAP1[] = {
            {GPIO_P1_27, IOCON_D_DEFAULT(3)}, // T0_CAP1
            {GPIO_P3_24, IOCON_D_DEFAULT(3)}, // T0_CAP1
    };
    static const T_LPC1788_IOCON_ConfigList pinsMAT0[] = {
            {GPIO_P1_28, IOCON_D_DEFAULT(3)}, // T0_MAT0
            {GPIO_P3_25, IOCON_D_DEFAULT(3)}, // T0_MAT0
    };
    static const T_LPC1788_IOCON_ConfigList pinsMAT1[] = {
            {GPIO_P1_29, IOCON_D_DEFAULT(3)}, // T0_MAT1
            {GPIO_P3_26, IOCON_D_DEFAULT(3)}, // T0_MAT0
    };
    HAL_DEVICE_REQUIRE_ONCE();
    // Register Timer0
    HALInterfaceRegister("Timer0", (T_halInterface *)&LPC1788_Timer0_Interface,
            0, 0);
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCAP[0], pinsCAP0,
            ARRAY_COUNT(pinsCAP0));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCAP[1], pinsCAP1,
            ARRAY_COUNT(pinsCAP1));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iMAT[0], pinsMAT0,
            ARRAY_COUNT(pinsMAT0));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iMAT[1], pinsMAT1,
            ARRAY_COUNT(pinsMAT1));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iMAT[2], 0, 0);
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iMAT[3], 0, 0);
}
...
void LPC1788_Timer1_Require(const T_LPC1788_Timer_Settings *aSettings)
{
    ...
    static const T_LPC1788_IOCON_ConfigList pinsMAT2[] = {
            {GPIO_P3_31, IOCON_D_DEFAULT(3)}, // T1_MAT2
    };
    HAL_DEVICE_REQUIRE_ONCE();
    // Register Timer1
    HALInterfaceRegister("Timer1", (T_halInterface *)&LPC1788_Timer1_Interface,
            0, 0);
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCAP[0], pinsCAP0,
            ARRAY_COUNT(pinsCAP0));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCAP[1], pinsCAP1,
    ...
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iMAT[1], pinsMAT1,
            ARRAY_COUNT(pinsMAT1));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iMAT[2], pinsMAT2,
...
void LPC1788_Timer2_Require(const T_LPC1788_Timer_Settings *aSettings)
{
    ...
    static const T_LPC1788_IOCON_ConfigList pinsMAT3[] = {
    ...
    HAL_DEVICE_REQUIRE_ONCE();
    // Register Timer2
    HALInterfaceRegister("Timer2", (T_halInterface *)&LPC1788_Timer2_Interface,
            0, 0);
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCAP[0], pinsCAP0,
            ARRAY_COUNT(pinsCAP0));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCAP[1], pinsCAP1,
    ...
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iMAT[2], pinsMAT2,
            ARRAY_COUNT(pinsMAT2));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iMAT[2], pinsMAT3,
...
void LPC1788_Timer3_Require(const T_LPC1788_Timer_Settings *aSettings)
{
    ...
    HAL_DEVICE_REQUIRE_ONCE();
    // Register Timer3
    HALInterfaceRegister("Timer3", (T_halInterface *)&LPC1788_Timer3_Interface,
            0, 0);
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCAP[0], pinsCAP0,
            ARRAY_COUNT(pinsCAP0));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCAP[1], pinsCAP1,
