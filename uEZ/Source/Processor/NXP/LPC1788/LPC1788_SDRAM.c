...
#include <uEZBSP.h>
#include <stdint.h>
#include "LPC1788_ExternalBus.h"
#include "LPC1788_SDRAM.h"
 ...
 **
 *****************************************************************************/
static uint32_t ILPC1788_SDRAM_QuickTest(TUInt32 aBaseAddress, TUInt32 aSize)
{
 ...
 **
 *****************************************************************************/
static uint32_t ILPC1788_SDRAM_FindCmdDelay(TUInt32 aBaseAddress, TUInt32 aSize)
{
...
void LPC1788_SDRAM_Init_32BitBus(const T_LPC1788_SDRAM_Configuration * aConfig)
{
    ...
    *dc |= 0x00080000;
    ...
    if (ILPC1788_SDRAM_FindCmdDelay(aConfig->iBaseAddress, aConfig->iSize)
            == 0x0) {
        UEZFailureMsg("SDRAM Init"); /* fatal error */
    }

    if (ILPC1788_SDRAM_FindFBClockDelay(aConfig->iBaseAddress, aConfig->iSize)
            == 0x0) {
        UEZFailureMsg("SDRAM Init"); /* fatal error */
    }
