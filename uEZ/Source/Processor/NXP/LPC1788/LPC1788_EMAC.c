 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1788 EMAC (Ethernet) Driver.
 * Implementation:
 *
 *-------------------------------------------------------------------------*/
 ...
 * Options:
 *---------------------------------------------------------------------------*/
#ifndef DEBUG_LPC1788_EMAC_STARTUP
#define DEBUG_LPC1788_EMAC_STARTUP  0
#endif

#ifndef EMAC_USE_BITBANG_MDIO_DETECT
/* Bit banging to the EMAC can be used to avoid the dread LPC1788 lock up
 * that occurs when a PHY is attempting to be communicated with and there
 * is NO oscillation.  The LPC1788 state machine locks up hard and only
...
typedef struct {
    ...
    T_LPC1788_EMAC_Settings iPinSettings;
    T_mdioBitBangSettings iMDIOSettings;
    TBool iIsDetected;
} T_LPC1788_EMAC_Workspace;
 ...
 * Inputs:
 *      T_LPC1788_EMAC_Workspace *p -- EMAC's Workspace
 *      TUInt32 aPhyReg             -- PHY register to write
 *      TUInt32 aValue              -- Value to write to register
 *---------------------------------------------------------------------------*/
static void IPHYWrite(T_LPC1788_EMAC_Workspace *p, TUInt8 PhyReg, TUInt32 Value)
{
    TUInt32 timeout;
 ...
 *      TUInt32 aValue              -- Value to write to register
 *---------------------------------------------------------------------------*/
static TUInt16 IPHYRead(T_LPC1788_EMAC_Workspace *p, TUInt8 PhyReg)
{
    TUInt32 timeout;
 ...
 *      T_LPC1788_EMAC_Workspace *p -- EMAC's Workspace
 *---------------------------------------------------------------------------*/
static T_uezError IEMACConfigPHY_National(T_LPC1788_EMAC_Workspace *p)
{
 ...
 *      T_LPC1788_EMAC_Workspace *p -- EMAC's Workspace
 *---------------------------------------------------------------------------*/
static T_uezError IEMACConfigPHY_Micrel(T_LPC1788_EMAC_Workspace *p)
{
    T_uezError error = UEZ_ERROR_NONE;
    TUInt32 timeout, regv;
#if DEBUG_LPC1788_EMAC_STARTUP
    TUInt32 reg;
#endif
    // auto negotiation, restart AN
    // RE_AN should be self cleared
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC1788_EMAC_InitRxDescriptors
 *---------------------------------------------------------------------------*
 * Description:
 *      Initialize Receive Descriptors
 *---------------------------------------------------------------------------*/
static void ILPC1788_EMAC_InitRxDescriptors(void)
{
    TUInt32 i;
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC1788_EMAC_InitTxDescriptors
 *---------------------------------------------------------------------------*
 * Description:
 *      Initialize Transmit Descriptors
 *---------------------------------------------------------------------------*/
void ILPC1788_EMAC_InitTxDescriptors(void)
{
    TUInt32 i;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_EMAC_InitializeWorkspace(void *aWorkspace)
{
    //    T_LPC1788_EMAC_Workspace *p = (T_LPC1788_EMAC_Workspace *)aWorkspace;

    // Nothing in this case
    G_LPC1788_EMAC = aWorkspace;
    G_LPC1788_EMAC->iIsDetected = EFalse;

    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_DetectEMAC
 *---------------------------------------------------------------------------*
 * Description:
 *      Detect if the EMAC can be found at all.
 * Inputs:
 *      T_LPC1788_EMAC_Workspace *p -- Workspace
 * Outputs:
 *      T_uezError                 -- Error code of UEZ_ERROR_NOT_FOUND if
 *                                    not detected, else UEZ_ERROR_NONE
 *---------------------------------------------------------------------------*/
#if EMAC_USE_BITBANG_MDIO_DETECT
static T_uezError LPC1788_EMAC_DetectEMAC(T_LPC1788_EMAC_Workspace *p)
{
    ...
    if ((pinMDC != GPIO_NONE) && (pinMDIO != GPIO_NONE)) {
        ...
        for (n=0; n<32; n++) {
#if DEBUG_LPC1788_EMAC_STARTUP
            printf("LPC1788_EMAC_DetectEMAC: PHY %d, Reg 2 = ", n);
#endif
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_EMAC_Configure(void *aWorkspace, T_EMACSettings *aSettings)
{
    T_uezError error = UEZ_ERROR_NONE;
    T_LPC1788_EMAC_Workspace *p = (T_LPC1788_EMAC_Workspace *)aWorkspace;
    ...
    // Because the LPC1788 has a problem with EMAC's that are missing,
    ...
    ILPC1788_EMAC_InitRxDescriptors();
    ILPC1788_EMAC_InitTxDescriptors();
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_CheckFrameReceived
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TBool LPC1788_EMAC_CheckFrameReceived(void *aWorkspace)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_StartReadFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TUInt16 LPC1788_EMAC_StartReadFrame(void *aWorkspace)
{
    TUInt16 RxLen;
    TUInt32 idx;
    T_LPC1788_EMAC_Workspace *p = (T_LPC1788_EMAC_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_EndReadFrame
 *---------------------------------------------------------------------------*
 * Description:
 *      End the last frame.
 * Inputs:
 *      void *aWorkspace          -- EMAC Controller's workspace
 *---------------------------------------------------------------------------*/
void LPC1788_EMAC_EndReadFrame(void *aWorkspace)
{
    TUInt32 idx;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_RequestSend
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      void *aWorkspace          -- EMAC Controller's workspace
 *---------------------------------------------------------------------------*/
void LPC1788_EMAC_RequestSend(void *aWorkspace)
{
    T_LPC1788_EMAC_Workspace *p = (T_LPC1788_EMAC_Workspace *)aWorkspace;
    TUInt32 idx;
 ...
 *---------------------------------------------------------------------------*/
void LPC1788_EMAC_DoSend(void *aWorkspace, TUInt16 aFrameSize)
{
    TUInt32 idx;
...
/*---------------------------------------------------------------------------*
 * Routine:  ILPC1788_WriteTxFrameData
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static void ILPC1788_WriteTxFrameData(
    T_LPC1788_EMAC_Workspace *p,
    TUInt16 aData)
{
    *(p->tptr++) = aData;
}
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_CopyToFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
void LPC1788_EMAC_CopyToFrame(void *aWorkspace, void *aSource, TUInt32 aSize)
{
    T_LPC1788_EMAC_Workspace *p = (T_LPC1788_EMAC_Workspace *)aWorkspace;
    TUInt16 *piSource;
    ...
    while (aSize > 0) {
        ILPC1788_WriteTxFrameData(p, *piSource++);
        aSize -= 2;
    }
}

/*---------------------------------------------------------------------------*
 * Routine:  ILPC1788_ReadRxFrameData
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
TUInt16 ILPC1788_ReadRxFrameData(T_LPC1788_EMAC_Workspace *p)
{
    return (*(p->rptr++));
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_CopyFromFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
void LPC1788_EMAC_CopyFromFrame(
    void *aWorkspace,
    void *aDestination,
    TUInt32 aSize)
{
    ...
    piDest = aDestination;
    while (aSize > 1) {
        *piDest++ = ILPC1788_ReadRxFrameData(p);
        aSize -= 2;
    }
    ...
    if (aSize) {
        ...
        *(TUInt8 *)piDest = (TUInt8)ILPC1788_ReadRxFrameData(p);
    }
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_ProcessInterrupt
 *---------------------------------------------------------------------------*
 * Description:
 *      An interrupt on this EMAC has been received.  Catch it and
 *      possible pass it on to a callback routine.
 *---------------------------------------------------------------------------*/
static void LPC1788_EMAC_ProcessInterrupt(void)
{
    T_LPC1788_EMAC_Workspace *p = G_LPC1788_EMAC;
    TUInt32 status;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_ISR
 *---------------------------------------------------------------------------*
 * Description:
 *      Stub to catch a EMAC interrupt.
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE( LPC1788_EMAC_ISR)
{
    IRQ_START();
    LPC1788_EMAC_ProcessInterrupt();
    IRQ_END();
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_EnableReceiveInterrupt
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *          or 0 for none.
 *---------------------------------------------------------------------------*/
void LPC1788_EMAC_EnableReceiveInterrupt(
    void *aWorkspace,
    ...
    void *aCallbackWorkspace)
{
    ...
    InterruptRegister(ENET_IRQn, (TISRFPtr)LPC1788_EMAC_ISR,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_DisableReceiveInterrupt
 *---------------------------------------------------------------------------*
 * Description:
 *      Disables receive interrupts from the EMAC.
 *---------------------------------------------------------------------------*/
void LPC1788_EMAC_DisableReceiveInterrupt(void *aWorkspace)
{
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_EMAC G_LPC1788_EMAC_Interface = {
    {
        "LPC1788 EMAC",
        0x0100,
        LPC1788_EMAC_InitializeWorkspace,
        sizeof(T_LPC1788_EMAC_Workspace), },

    LPC1788_EMAC_Configure,
    LPC1788_EMAC_CheckFrameReceived,
    LPC1788_EMAC_StartReadFrame,
    LPC1788_EMAC_EndReadFrame,
    LPC1788_EMAC_RequestSend,
    LPC1788_EMAC_DoSend,
    LPC1788_EMAC_CopyToFrame,
    LPC1788_EMAC_CopyFromFrame,

    // uEZ v1.10
    LPC1788_EMAC_EnableReceiveInterrupt,
    LPC1788_EMAC_DisableReceiveInterrupt,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_EMAC_RMII_Require
 *---------------------------------------------------------------------------*
 * Description:
 *      Creates the EMAC HAL driver for the LPC1788 device.  Also looks
 ...
 * Inputs:
 *      T_LPC1788_EMAC_Settings *aSettings -- Settings to configure EMAC as
 *      TBool *aIsFound -- REturned flag if found
 *---------------------------------------------------------------------------*/
void LPC1788_EMAC_RMII_Require(const T_LPC1788_EMAC_Settings *aSettings)
{
    T_LPC1788_EMAC_Workspace *p;

    static const T_LPC1788_IOCON_ConfigList pinsTX_EN[] = { {
        GPIO_P1_4,
        IOCON_D_DEFAULT(1) }, // ENET_TX_EN
        };
    static const T_LPC1788_IOCON_ConfigList pinsTXD3[] = { {
        GPIO_P1_3,
        IOCON_D_DEFAULT(1) }, // ENET_TXD[3]
        };
    static const T_LPC1788_IOCON_ConfigList pinsTXD2[] = { {
        GPIO_P1_2,
        IOCON_D_DEFAULT(1) }, // ENET_TXD[2]
        };
    static const T_LPC1788_IOCON_ConfigList pinsTXD1[] = { {
        GPIO_P1_1,
        IOCON_D_DEFAULT(1) }, // ENET_TXD[1]
        };
    static const T_LPC1788_IOCON_ConfigList pinsTXD0[] = { {
        GPIO_P1_0,
        IOCON_D_DEFAULT(1) }, // ENET_TXD[0]
        };
    static const T_LPC1788_IOCON_ConfigList pinsTX_ER[] = { {
        GPIO_P1_5,
        IOCON_D_DEFAULT(1) }, // ENET_TX_ER
        };
    static const T_LPC1788_IOCON_ConfigList pinsTX_CLK[] = { {
        GPIO_P1_6,
        IOCON_D_DEFAULT(1) }, // ENET_TX_CLK
        };
    static const T_LPC1788_IOCON_ConfigList pinsRX_DV[] = { {
        GPIO_P1_13,
        IOCON_D_DEFAULT(1) }, // ENET_RX_DV
        };
    static const T_LPC1788_IOCON_ConfigList pinsRXD3[] = { {
        GPIO_P1_12,
        IOCON_D_DEFAULT(1) }, // ENET_RXD[3]
        };
    static const T_LPC1788_IOCON_ConfigList pinsRXD2[] = { {
        GPIO_P1_11,
        IOCON_D_DEFAULT(1) }, // ENET_RXD[2]
        };
    static const T_LPC1788_IOCON_ConfigList pinsRXD1[] = { {
        GPIO_P1_10,
        IOCON_D_DEFAULT(1) }, // ENET_RXD[1]
        };
    static const T_LPC1788_IOCON_ConfigList pinsRXD0[] = { {
        GPIO_P1_9,
        IOCON_D_DEFAULT(1) }, // ENET_RXD[0]
        };
    static const T_LPC1788_IOCON_ConfigList pinsRX_ER[] = { {
        GPIO_P1_14,
        IOCON_D_DEFAULT(1) }, // ENET_RX_ER
        };
    static const T_LPC1788_IOCON_ConfigList pinsRX_CLK[] = { {
        GPIO_P1_15,
        IOCON_D_DEFAULT(1) }, // ENET_RX_CLK
        };
    static const T_LPC1788_IOCON_ConfigList pinsCOL[] = { {
        GPIO_P1_7,
        IOCON_D_DEFAULT(1) }, // ENET_COL
        };
    static const T_LPC1788_IOCON_ConfigList pinsCRS[] = { {
        GPIO_P1_8,
        IOCON_D_DEFAULT(1) }, // ENET_CRS
        };
    // Use the MDC and MDIO pins as EMAC pins
    static const T_LPC1788_IOCON_ConfigList pinsMDC[] = {
        {   GPIO_P1_16, IOCON_D_DEFAULT(1)}, // ENET_MDC
        {   GPIO_P2_8, IOCON_D_DEFAULT(4)}, // ENET_MDC
    };
    static const T_LPC1788_IOCON_ConfigList pinsMDIO[] = {
    ...
    // Register EMAC
    HALInterfaceRegister("EMAC", (T_halInterface *)&G_LPC1788_EMAC_Interface, 0,
        (T_halWorkspace **)&p);
    p->iPinSettings = *aSettings;
    LPC1788_IOCON_ConfigPin(aSettings->iTX_EN, pinsTX_EN,
        ARRAY_COUNT(pinsTX_EN));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iTXD3, pinsTXD3,
        ARRAY_COUNT(pinsTXD3));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iTXD2, pinsTXD2,
        ARRAY_COUNT(pinsTXD2));
    LPC1788_IOCON_ConfigPin(aSettings->iTXD1, pinsTXD1, ARRAY_COUNT(pinsTXD1));
    LPC1788_IOCON_ConfigPin(aSettings->iTXD0, pinsTXD0, ARRAY_COUNT(pinsTXD0));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iTX_ER, pinsTX_ER,
        ARRAY_COUNT(pinsTX_ER));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iTX_CLK, pinsTX_CLK,
        ARRAY_COUNT(pinsTX_CLK));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iRX_DV, pinsRX_DV,
        ARRAY_COUNT(pinsRX_DV));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iRXD3, pinsRXD3,
        ARRAY_COUNT(pinsRXD3));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iRXD2, pinsRXD2,
        ARRAY_COUNT(pinsRXD2));
    LPC1788_IOCON_ConfigPin(aSettings->iRXD1, pinsRXD1, ARRAY_COUNT(pinsRXD1));
    LPC1788_IOCON_ConfigPin(aSettings->iRXD0, pinsRXD0, ARRAY_COUNT(pinsRXD0));
    LPC1788_IOCON_ConfigPin(aSettings->iRX_ER, pinsRX_ER,
        ARRAY_COUNT(pinsRX_ER));
    LPC1788_IOCON_ConfigPin(aSettings->iRX_CLK, pinsRX_CLK,
        ARRAY_COUNT(pinsRX_CLK));
    LPC1788_IOCON_ConfigPinOrNone(aSettings->iCOL, pinsCOL,
        ARRAY_COUNT(pinsCOL));
    LPC1788_IOCON_ConfigPin(aSettings->iCRS, pinsCRS, ARRAY_COUNT(pinsCRS));
    LPC1788_IOCON_ConfigPin(aSettings->iMDC, pinsMDC, ARRAY_COUNT(pinsMDC));
    LPC1788_IOCON_ConfigPin(aSettings->iMDIO, pinsMDIO, ARRAY_COUNT(pinsMDIO));
    ...
    if (LPC1788_EMAC_DetectEMAC(p) == UEZ_ERROR_NONE)
        p->iIsDetected = ETrue;
#else
    p->iIsDetected = ETrue;
#endif
