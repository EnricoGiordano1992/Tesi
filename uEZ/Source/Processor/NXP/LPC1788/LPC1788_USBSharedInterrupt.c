 ...
 * Function Prototypes:
 *---------------------------------------------------------------------------*/
extern void LPC1788_InterruptProcess_USBDevice(void);
extern void LPC1788_InterruptProcess_USBHost(void);

/*---------------------------------------------------------------------------*
 * Routine:  ILPC1788_USB_InterruptVector
 *---------------------------------------------------------------------------*
 * Description:
 *      Interrupt Vector routine that handles both USB device and USB host
 *      (if they are compiled in)
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE(ILPC1788_USB_InterruptVector)
{
    IRQ_START();

    // Chain together these functions, one for the device, one for the host
    if (LPC_USB->USBClkSt & (1<<1))
        LPC1788_InterruptProcess_USBDevice();
    LPC1788_InterruptProcess_USBHost();
