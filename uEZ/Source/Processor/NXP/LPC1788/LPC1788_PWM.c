 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1788 PWMs Interface.
 * Implementation:
...
#define PWM_TCR_MASTER_DISABLE  (1<<4)  // PWM0 only!
    ...
    TVUInt32 iCTCR; // 0x70
} T_LPC1788_PWM_Registers;

typedef struct {
    const HAL_PWM *iHAL;
    T_LPC1788_PWM_Registers *iReg;
    TUInt32 iPeriod;
} T_LPC1788_PWM_Workspace;

static T_uezError LPC1788_PWM_SetMatchRegister(
        void *aWorkspace,
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_PWM_SetMaster(
        void *aWorkspace,
        TUInt32 aPeriod,
        TUInt8 aMatchRegister)
{
    T_LPC1788_PWM_Workspace *p = (T_LPC1788_PWM_Workspace *)aWorkspace;
    T_LPC1788_PWM_Registers *r = p->iReg;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_PWM_SetMatchRegister
 *---------------------------------------------------------------------------*
 * Description:
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_PWM_EnableSingleEdgeOutput
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_PWM_EnableSingleEdgeOutput(
        void *aWorkspace,
        TUInt8 aMatchRegister)
{
    T_LPC1788_PWM_Workspace *p = (T_LPC1788_PWM_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_PWM_DisableOutput
 *---------------------------------------------------------------------------*
 * Description:
 *      PWM is going to have a single edge output (high until matches, then
 *      low, reset counter goes back to high).
 ...
 *      T_uezError              -- UEZ_ERROR_NONE
 *---------------------------------------------------------------------------*/
static T_uezError LPC1788_PWM_DisableOutput(
        void *aWorkspace,
        TUInt8 aMatchRegister)
{
    T_LPC1788_PWM_Workspace *p = (T_LPC1788_PWM_Workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_PWM_PWM0_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 PWM0 workspace.
 * Inputs:
 *      void *aW                    -- Particular PWM workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_PWM_PWM0_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1788_PWM_Workspace *p = (T_LPC1788_PWM_Workspace *)aWorkspace;
    p->iReg = (T_LPC1788_PWM_Registers *)LPC_PWM0_BASE;
    LPC1788PowerOn(1<<5);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_PWM_PWM1_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 PWM1 workspace.
 * Inputs:
 *      void *aW                    -- Particular PWM workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_PWM_PWM1_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1788_PWM_Workspace *p = (T_LPC1788_PWM_Workspace *)aWorkspace;
    p->iReg = (T_LPC1788_PWM_Registers *)LPC_PWM1_BASE;
    LPC1788PowerOn(1<<6);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_PWM PWM_LPC1788_Port0_Interface = {
    {
    "NXP:LPC1788:PWM0",
    0x0100,
    LPC1788_PWM_PWM0_InitializeWorkspace,
    sizeof(T_LPC1788_PWM_Workspace),
    },

    LPC1788_PWM_SetMaster,
    LPC1788_PWM_SetMatchRegister,
    LPC1788_PWM_EnableSingleEdgeOutput,
    LPC1788_PWM_DisableOutput,
};

const HAL_PWM PWM_LPC1788_Port1_Interface = {
    {
    "NXP:LPC1788:PWM1",
    0x0100,
    LPC1788_PWM_PWM1_InitializeWorkspace,
    sizeof(T_LPC1788_PWM_Workspace),
    },
 ...
 *---------------------------------------------------------------------------*/
void LPC1788_PWM0_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE();
    // Register PWM0
    HALInterfaceRegister("PWM0",
            (T_halInterface *)&PWM_LPC1788_Port0_Interface, 0, 0);

}

void LPC1788_PWM0_1_Require(T_uezGPIOPortPin aPortPin)
{
    ...
    LPC1788_IOCON_ConfigPin(aPortPin, pins, ARRAY_COUNT(pins));
}

void LPC1788_PWM0_2_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM0_3_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM0_4_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM0_5_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM0_6_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM0_CAP0_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM1_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE();
    // Register PWM1
    HALInterfaceRegister("PWM1",
            (T_halInterface *)&PWM_LPC1788_Port1_Interface, 0, 0);
}

void LPC1788_PWM1_1_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM1_2_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM1_3_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM1_4_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM1_5_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM1_6_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM1_CAP0_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
...
void LPC1788_PWM1_CAP1_Require(T_uezGPIOPortPin aPortPin)
{
    static const T_LPC1788_IOCON_ConfigList pins[] = {
