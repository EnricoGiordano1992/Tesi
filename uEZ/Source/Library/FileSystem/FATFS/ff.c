...
/*-----------------------------------------------------------------------*/
/* Find one of the drives by number                                      */
/*-----------------------------------------------------------------------*/
FRESULT f_get_drive(BYTE aDrive, FATFS **p_fs)
{
    if (aDrive < _DRIVES) {
        *p_fs = FatFs[aDrive];
        return FR_OK;
...
/*-----------------------------------------------------------------------*/
/* Ensure the drive is synchronized                                      */
/*-----------------------------------------------------------------------*/
FRESULT f_drive_sync(BYTE aDrive)
{
    FATFS *p_fs;
    FRESULT res;

    res = f_get_drive(aDrive, &p_fs);
    if (res == FR_OK)
        res = sync(p_fs);
...
)
{
	...
	*rfs = fs = FatFs[drv];					/* Returen pointer to the corresponding file system object */
	...
	if (maxclust >= 0xFF7) fmt = FS_FAT16;		/* Number of clusters >= 0xFF5 */
	if (maxclust >= 0xFFF7) fmt = FS_FAT32;		/* Number of clusters >= 0xFFF5 */
