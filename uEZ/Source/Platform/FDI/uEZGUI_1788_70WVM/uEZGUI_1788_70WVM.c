...
#include <Source/Devices/Touchscreen/Sitronix/ST1232/ST1232TouchScreen.h>
...
static T_uezError UEZPlatform_I2C1_Reset(void)
{
    printf("I2C1 was reset\n");
    //Set all devices on the board to be in reset
    UEZGPIOSetMux(GPIO_P1_18, 0);
    UEZGPIOOutput(GPIO_P1_18);
    UEZGPIOClear(GPIO_P1_18);
    //Set any pins to GPIO lows that need to be set, like on the 70WVM

    //delay
    UEZTaskDelay(10);
    //Set pins back to the need configuration

    //take devices out of reset.
    UEZGPIOSet(GPIO_P1_18);
...
void UEZPlatform_I2C1_Require(void)
{
    ...
    I2C_Generic_Create("I2C1", "I2C1", UEZPlatform_I2C1_Reset);
}
...
void UEZPlatform_MCI_Require(void)
{
    const T_LPC1788_MCI_Pins pins = {
        ...
        GPIO_P1_12, // DAT3
        GPIO_P0_19, // CLK
        //Support on board high speed sdcard
        //to support the UEZGUI-EXP-DK use GPIO_P1_3 for CMD
        //GPIO_P1_3, //CMD
        GPIO_P0_20,  // CMD
    ...
    UEZGPIOSetMux(GPIO_P0_21, 0);
    UEZGPIOOutput(GPIO_P0_21);
    UEZGPIOSet(GPIO_P0_21);
}
...
/*---------------------------------------------------------------------------*
 * Routine:  UEZGUI70WVM_AudioMixerCallback
 *---------------------------------------------------------------------------*
 * Description:
 *      
 *---------------------------------------------------------------------------*/
static T_uezError UEZGUI70WVM_AudioMixerCallback(
...
void UEZPlatform_AudioMixer_Require(void)
{
    ...
    UEZAudioMixerRegister(UEZ_AUDIO_MIXER_OUTPUT_ONBOARD_SPEAKER, &UEZGUI70WVM_AudioMixerCallback);
...
void UEZPlatform_Touchscreen_Require(void)
{
    ...
    Touchscreen_ST1232_Create("Touchscreen", "I2C1", GPIO_P2_19, GPIO_P1_18);
}
...
void UEZPlatform_WirelessNetwork0_Require(void)
{
    ...
    // SPI Settings
        ...
        GPIO_P1_6,  // iProgramMode = P1.6_MCIDAT0_PWM0.4
        GPIO_P5_0,  // iSPIMode = WIFI_MODE 
        GPIO_P1_5,  // iSRSTn = WIFI_SRSTn AND'd with RESET_OUTn
        GPIO_P5_1,  // iWIFIFactoryRestore = WIFI_FAC_RST
        };
...
void UEZPlatform_AudioCodec_Require(void)
{
    ...
    UEZI2COpen("I2C2", &i2c2);

    // Both DAC and I2S are on
    data[0] = 0x1C;
    UEZI2CWrite(
        i2c2,
        0x7C,
    ...
    // DAC volume set to no clipping max
    data[0] = 0x73;
    UEZI2CWrite(
        i2c2,
        0x7C,
    ...
    // I2S volume set to less than max
    data[0] = 0x98;
    UEZI2CWrite(
        i2c2,
        0x7C,
