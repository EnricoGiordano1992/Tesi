...
void UEZPlatform_I2C2_Require(void)
{
    ...
    LPC2478_GPIO0_Require();
    LPC2478_I2C2_Require(GPIO_P4_20, GPIO_P4_21);
...
void UEZPlatform_SSP0_Require(void)
{
    ...
    LPC2478_SSP0_Require(GPIO_P0_15, GPIO_P0_17, GPIO_P0_18, GPIO_NONE);
...
void UEZPlatform_Backlight_Require(void)
{
    ...
    DEVICE_CREATE_ONCE();
    LPC2478_PWM1_1_Require(GPIO_P1_18);
...
void UEZPlatform_LCD_Require(void)
{
...
#elif (UEZ_DEFAULT_LCD_CONFIG==LCD_CONFIG_KOE_TX13D06VM2BAA)
    LCD_TX13D06VM2BAA_Create("LCD", GPIO_P2_4, GPIO_P2_0, GPIO_P2_11, GPIO_NONE, GPIO_NONE);
#endif
    ...
    // Open the LCD now! Before the USB Device initializes!
    // This is a fix to the initialization order of the LCD in reference to the
    // order of the USB Device driver.  LCD *MUST* be initialized before USB Device!
    // The problem is with the USB_ATX_INT interrupt signal in USBIntSt register.
    // If the USB Device tries to come up while P1.27 is SetMux to 1 (it is both
    // LCDVD[12], LCDVD[20], and USB_INT1 rolled into 1), the USB_ATX_INT will
    // be set and keep being set.  The solution is to make sure the LCD is
    // fully initialized so the USB_INT1 pin function is overridden.  Since any
    // user requires LCD is going to open the LCD, this is a harmless command
    // that powers up the LCD.  It will show nothing until the LCD backlight
    // is turned on.  It is left off.
    // -- lshields 4/1/2013
    UEZLCDOpen("LCD", &lcd);
}
...
void UEZPlatform_EEPROM_I2C_Require(void)
{
    ...
    EEPROM_Generic_I2C_Create("EEPROM0", "I2C2", 0xA8>>1, &eeprom_config);
}
...
void UEZPlatform_USBDevice_Require(void)
{
    const T_LPC2478_USBDevice_Settings portBSettings = {
            ...
            GPIO_NONE,      // USB_D-2          = USB2_DM (not a GPIO port pin)
