...
void UEZPlatform_LCD_Require(void)
{
    const T_LPC17xx_40xx_LCDController_Pins pins = {
            GPIO_P2_0,  // LCD_PWR -- GPIO controlled
            GPIO_P2_2,  // LCD_DCLK
...
{
    // If current rev, do only Port A (expansion board).
...
/*---------------------------------------------------------------------------*
 * Routine:  UEZGUI43WQS_AudioMixerCallback
 *---------------------------------------------------------------------------*
 * Description:
 *
 *---------------------------------------------------------------------------*/
static T_uezError UEZGUI43WQS_AudioMixerCallback(
...
void UEZPlatform_AudioMixer_Require(void)
{
    ...
    UEZAudioMixerRegister(UEZ_AUDIO_MIXER_OUTPUT_ONBOARD_SPEAKER, &UEZGUI43WQS_AudioMixerCallback);
...
/*---------------------------------------------------------------------------*
 * Routine:  UEZPlatform_SWO_Require
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the SWO debug terminal output stream.
 *---------------------------------------------------------------------------*/
void UEZPlatform_SWO_Require(void)
{
    DEVICE_CREATE_ONCE();
    Stream_SWO_CortexM_Create("SWO");
}
