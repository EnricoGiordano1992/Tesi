 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the Toshiba LTA057A347F LCD.
 *-------------------------------------------------------------------------*/
...
} T_LTA057A347FWorkspace;

/*---------------------------------------------------------------------------*
 * Globals:
 *---------------------------------------------------------------------------*/
static T_LCDControllerSettings LCD_LTA057A347F_settings;

static T_LCDControllerSettings LCD_LTA057A347F_params16bit = {
    LCD_ADVANCED_TFT,
...
static T_LCDControllerSettings LCD_LTA057A347F_paramsI15bit = {
    LCD_ADVANCED_TFT,
...
#if 0 // testing
static T_LCDControllerSettings LCD_LTA057A347F_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
#else // testing
static const T_LCDControllerSettings LCD_LTA057A347F_params8bit = {
    LCD_ADVANCED_TFT,
    LCD_COLOR_RES_8,
...
static T_uezLCDConfiguration LCD_LTA057A347F_configuration_16Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_LTA057A347F_configuration_I15Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
...
static T_uezLCDConfiguration LCD_LTA057A347F_configuration_8Bit = {
    RESOLUTION_X,
    RESOLUTION_Y,
 ...
 * Prototypes:
 *---------------------------------------------------------------------------*/
extern const DEVICE_LCD LCD_Toshiba_LTA057A347F_Interface;

/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_InitializeWorkspace_16Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LTA057A347F LCD.
 * Inputs:
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_InitializeWorkspace_I15Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LTA057A347F LCD.
 * Inputs:
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_InitializeWorkspace_8Bit
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup workspace for LTA057A347F LCD.
 * Inputs:
 ...
 *                                  UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_Open(void *aW)
{
    T_LTA057A347FWorkspace *p = (T_LTA057A347FWorkspace *)aW;
    ...
    if (p->aNumOpen == 1) {
        ...
        switch (p->iConfiguration->iColorDepth) {
            case UEZLCD_COLOR_DEPTH_8_BIT:
                LCD_LTA057A347F_settings = LCD_LTA057A347F_params8bit;
                break;
            default:
            case UEZLCD_COLOR_DEPTH_16_BIT:
                LCD_LTA057A347F_settings = LCD_LTA057A347F_params16bit;
                break;
            case UEZLCD_COLOR_DEPTH_I15_BIT:
                LCD_LTA057A347F_settings = LCD_LTA057A347F_paramsI15bit;
                break;
        }
        LCD_LTA057A347F_settings.iBaseAddress = p->iBaseAddress;

        return (*plcdc)->Configure(plcdc, &LCD_LTA057A347F_settings);
    }
 ...
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_Close(void *aW)
{
    T_LTA057A347FWorkspace *p = (T_LTA057A347FWorkspace *)aW;
    p->aNumOpen--;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_Configure(
            void *aW,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_GetInfo
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_GetInfo(void *aW, T_uezLCDConfiguration *aConfiguration)
{
    T_LTA057A347FWorkspace *p = (T_LTA057A347FWorkspace *)aW;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_GetFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_GetFrame(
            void *aW,
            TUInt32 aFrame,
            void **aFrameBuffer)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_ShowFrame
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_ShowFrame(void *aW, TUInt32 aFrame)
{
    T_LTA057A347FWorkspace *p = (T_LTA057A347FWorkspace *)aW;
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_On(void *aW)
{
 ...
 *      T_uezError               -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_Off(void *aW)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_SetBacklightLevel(void *aW, TUInt32 aLevel)
{
    T_LTA057A347FWorkspace *p = (T_LTA057A347FWorkspace *)aW;
    TUInt16 level;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_GetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                  no backlight for LCD.
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LTA057A347F_GetBacklightLevel(
        void *aW, 
        TUInt32 *aLevel,
        TUInt32 *aNumLevels)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LCD_LTA057A347F_SetPaletteColor(
                    void *aWorkspace,
                    TUInt32 aColorIndex,
                    TUInt16 aRed,
                    TUInt16 aGreen,
                    TUInt16 aBlue)
{
    T_LTA057A347FWorkspace *p = (T_LTA057A347FWorkspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LCD_LTA057A347F_WaitForVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static T_uezError LCD_LTA057A347F_WaitForVerticalSync(
    void *aWorkspace,
    TUInt32 aTimeout)
{
    T_LTA057A347FWorkspace *p = (T_LTA057A347FWorkspace *)aWorkspace;
    ...
    (*p_hal)->EnableVerticalSync(p_hal, LCD_LTA057A347F_VerticalSyncCallback, p);
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const DEVICE_LCD LCD_Toshiba_LTA057A347F_Interface_16Bit = {
	{
	    // Common interface
	    "LCD:Toshiba_LTA057A347F:16Bit",
	    0x0100,
	    LCD_LTA057A347F_InitializeWorkspace_16Bit,
	    sizeof(T_LTA057A347FWorkspace),
	},
	
    // Functions
    LCD_LTA057A347F_Open,
    LCD_LTA057A347F_Close,
    LCD_LTA057A347F_Configure,
    LCD_LTA057A347F_GetInfo,
    LCD_LTA057A347F_GetFrame,
    LCD_LTA057A347F_ShowFrame,
    LCD_LTA057A347F_On,
    LCD_LTA057A347F_Off,
    LCD_LTA057A347F_SetBacklightLevel,
    LCD_LTA057A347F_GetBacklightLevel,
    LCD_LTA057A347F_SetPaletteColor,
    // v2.04
    LCD_LTA057A347F_WaitForVerticalSync,
};

const DEVICE_LCD LCD_Toshiba_LTA057A347F_Interface_I15Bit = {
	{
	    // Common interface
	    "LCD:Toshiba_LTA057A347F:I15Bit",
	    0x0100,
	    LCD_LTA057A347F_InitializeWorkspace_I15Bit,
	    sizeof(T_LTA057A347FWorkspace),
	},
...
const DEVICE_LCD LCD_Toshiba_LTA057A347F_Interface_8Bit = {
	{
	    // Common interface
	    "LCD:Toshiba_LTA057A347F:8Bit",
	    0x0100,
	    LCD_LTA057A347F_InitializeWorkspace_8Bit,
	    sizeof(T_LTA057A347FWorkspace),
	},
...
// List of interfaces
const T_uezDeviceInterface *LCD_Toshiba_LTA057A347F_InterfaceArray[] = {
    0,      // 1 bit
    0,      // 2 bit
    0,      // 4 bit
    (T_uezDeviceInterface *)&LCD_Toshiba_LTA057A347F_Interface_8Bit,      // 8 bit
    (T_uezDeviceInterface *)&LCD_Toshiba_LTA057A347F_Interface_16Bit,      // 16 bit
    (T_uezDeviceInterface *)&LCD_Toshiba_LTA057A347F_Interface_I15Bit,      // I15 bit
    0,      // 24 bit
};
