 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1788 LCD Controller.
 * Implementation:
...
static T_workspace *G_lpc1788_lcdWorkspace;

/*---------------------------------------------------------------------------*
 * Prototypes:
 *---------------------------------------------------------------------------*/
static void ILPC1788_ProcessVerticalSync(T_workspace *p);
T_uezError LPC1788_LCDController_DisableVerticalSync(void *aWorkspace);
 ...
 *      in ILPC1788_ProcessVerticalSync().
 *---------------------------------------------------------------------------*/
IRQ_ROUTINE(LCDControllerVerticalIRQ)
{
    IRQ_START();
    ILPC1788_ProcessVerticalSync(G_lpc1788_lcdWorkspace);
    IRQ_END();
}

/*---------------------------------------------------------------------------*
 * Routine:  ILPC1788_ProcessVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 *      Interrupt handler for LCD interrupts.  Process the interrupt
 *      in ILPC1788_ProcessVerticalSync().
 ...
 *---------------------------------------------------------------------------*/
void ILPC1788_ProcessVerticalSync(T_workspace *p)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_LCDController_InitializeWorkspace(void *aWorkspace)
{
    T_workspace *p = (T_workspace *)aWorkspace;
    p->iHAL = &LCDController_LPC1788_Interface;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_Configure
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_LCDController_Configure(
        void *aWorkspace,
        T_LCDControllerSettings *aSettings)
{
    ...
    // Give LCD highest priority on the LPC1788
    LPC_SC->MATRIXARB = 0x00000C09 | (1<<16);
    return UEZ_ERROR_NONE;
}

/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_On
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_LCDController_On(void *aWorkspace)
{
    T_workspace *p = (T_workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_Off
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_LCDController_Off(void *aWorkspace)
{
    T_workspace *p = (T_workspace *)aWorkspace;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_SetBacklightLevel
 *---------------------------------------------------------------------------*
 * Description:
 *      Control the backlight level.  For LPC1788, this is not a feature.
 * Inputs:
 *      void *aWorkspace          -- LCD Controller's workspace
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_LCDController_SetBacklightLevel(
        void *aWorkspace,
        TUInt32 aLevel)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_SetBaseAddr
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                 -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_LCDController_SetBaseAddr(
        void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_SetPaletteColor
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_LCDController_SetPaletteColor(
        void *aWorkspace,
        TUInt32 aColorIndex,
        TUInt16 aRed,
        TUInt16 aGreen,
...
/*---------------------------------------------------------------------------*
 * Routine:  LCDController_LPC1788_ConfigurePowerPin
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                   -- If successful, returns UEZ_ERROR_NONE.
 *---------------------------------------------------------------------------*/
T_uezError LCDController_LPC1788_ConfigurePowerPin(
            void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_EnableVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
    ...
    void *aCallbackWorkspace)
{
    ...
    if (p->iVerticalSyncCallbackFunc)
        LPC1788_LCDController_DisableVerticalSync(aWorkspace);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_LCDController_DisableVerticalSync
 *---------------------------------------------------------------------------*
 * Description:
 ...
 * HAL Interface table:
 *---------------------------------------------------------------------------*/
const HAL_LCDController LCDController_LPC1788_Interface = {
        {
        "LPC1788 LCDController",
        0x0100,
        LPC1788_LCDController_InitializeWorkspace,
        sizeof(T_workspace),
        },
        ...
        LPC1788_LCDController_Configure,
        LPC1788_LCDController_On,
        LPC1788_LCDController_Off,
        LPC1788_LCDController_SetBacklightLevel,
        LPC1788_LCDController_SetBaseAddr,
        LPC1788_LCDController_SetPaletteColor,
        LPC1788_LCDController_EnableVerticalSync,
        LPC1788_LCDController_DisableVerticalSync
};
...
void LPC1788_LCDController_Require(const T_LPC1788_LCDController_Pins *aPins)
{
    ...
    HALInterfaceRegister("LCDController",
            (T_halInterface *)&LCDController_LPC1788_Interface, 0, &p_lcd);
    LPC1788_IOCON_ConfigPinOrNone(aPins->iPWR, pinsPWR,
            ARRAY_COUNT(pinsPWR));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iLE, pinsLE,
            ARRAY_COUNT(pinsLE));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iDCLK, pinsDCLK,
            ARRAY_COUNT(pinsDCLK));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iFP, pinsFP,
            ARRAY_COUNT(pinsFP));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iENAB_M, pinsENAB,
            ARRAY_COUNT(pinsENAB));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iLP, pinsLP,
            ARRAY_COUNT(pinsLP));

    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[0], pinsVD0,
            ARRAY_COUNT(pinsVD0));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[1], pinsVD1,
            ARRAY_COUNT(pinsVD1));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[2], pinsVD2,
            ARRAY_COUNT(pinsVD2));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[3], pinsVD3,
            ARRAY_COUNT(pinsVD3));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[4], pinsVD4,
            ARRAY_COUNT(pinsVD4));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[5], pinsVD5,
            ARRAY_COUNT(pinsVD5));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[6], pinsVD6,
            ARRAY_COUNT(pinsVD6));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[7], pinsVD7,
            ARRAY_COUNT(pinsVD7));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[8], pinsVD8,
            ARRAY_COUNT(pinsVD8));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[9], pinsVD9,
            ARRAY_COUNT(pinsVD9));

    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[10], pinsVD10,
            ARRAY_COUNT(pinsVD10));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[11], pinsVD11,
            ARRAY_COUNT(pinsVD11));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[12], pinsVD12,
            ARRAY_COUNT(pinsVD12));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[13], pinsVD13,
            ARRAY_COUNT(pinsVD13));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[14], pinsVD14,
            ARRAY_COUNT(pinsVD14));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[15], pinsVD15,
            ARRAY_COUNT(pinsVD15));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[16], pinsVD16,
            ARRAY_COUNT(pinsVD16));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[17], pinsVD17,
            ARRAY_COUNT(pinsVD17));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[18], pinsVD18,
            ARRAY_COUNT(pinsVD18));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[19], pinsVD19,
            ARRAY_COUNT(pinsVD19));

    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[20], pinsVD20,
            ARRAY_COUNT(pinsVD20));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[21], pinsVD21,
            ARRAY_COUNT(pinsVD21));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[22], pinsVD22,
            ARRAY_COUNT(pinsVD22));
    LPC1788_IOCON_ConfigPinOrNone(aPins->iVD[23], pinsVD23,
            ARRAY_COUNT(pinsVD23));

    LPC1788_IOCON_ConfigPinOrNone(aPins->iCLKIN, pinsCLKIN,
    ...
    if (aPins->iPowerPin != GPIO_NONE) {
        ...
        LCDController_LPC1788_ConfigurePowerPin(p_lcd,
