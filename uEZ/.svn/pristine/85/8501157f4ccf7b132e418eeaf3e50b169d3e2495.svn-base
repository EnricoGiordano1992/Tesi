 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1788 GP DMA controller.
 * Implementation:
 *
 *-------------------------------------------------------------------------*/
...
} T_LPC1788_GPDMA_Workspace;

/*---------------------------------------------------------------------------*
 * Forward declaration:
 *---------------------------------------------------------------------------*/
static void LPC1788_GPDMA_Enable(void *aWorkspace);
/*static void LPC1788_GPDMA_Disable(void *aWorkspace);*/
static TBool LPC1788_GPDMA_ChannelIsEnabled(void *aWorkspace);
static void IGPDMAProcessInterrupt(T_LPC1788_GPDMA_Workspace *p);
IRQ_ROUTINE( IGPDMA)
;
...
static T_LPC1788_GPDMA_Workspace * G_workspaceGPDMAChannel[8];

T_uezError LPC1788_GPDMA_InitializeWorkspace_ChannelX(
    void *aWorkspace,
    TUInt8 aChannel)
{
    T_LPC1788_GPDMA_Workspace *p = (T_LPC1788_GPDMA_Workspace *)aWorkspace;

    // Setup GPDMA peripheral
    LPC1788_GPDMA_Enable(aWorkspace);
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_GPDMA_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 GPDMA iChannel workspaces.
 * Inputs:
 *      void *aW                    -- Particular GPDMA workspace
 * Outputs:
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_GPDMA_InitializeWorkspace_Channel0(void *aWorkspace)
{
    return LPC1788_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
        GPDMA_CHANNEL0);
}
T_uezError LPC1788_GPDMA_InitializeWorkspace_Channel1(void *aWorkspace)
{
    return LPC1788_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
        GPDMA_CHANNEL1);
}
T_uezError LPC1788_GPDMA_InitializeWorkspace_Channel2(void *aWorkspace)
{
    return LPC1788_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
        GPDMA_CHANNEL2);
}
T_uezError LPC1788_GPDMA_InitializeWorkspace_Channel3(void *aWorkspace)
{
    return LPC1788_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
        GPDMA_CHANNEL3);
}
T_uezError LPC1788_GPDMA_InitializeWorkspace_Channel4(void *aWorkspace)
{
    return LPC1788_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
        GPDMA_CHANNEL4);
}
T_uezError LPC1788_GPDMA_InitializeWorkspace_Channel5(void *aWorkspace)
{
    return LPC1788_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
        GPDMA_CHANNEL5);
}
T_uezError LPC1788_GPDMA_InitializeWorkspace_Channel6(void *aWorkspace)
{
    return LPC1788_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
        GPDMA_CHANNEL6);
}
T_uezError LPC1788_GPDMA_InitializeWorkspace_Channel7(void *aWorkspace)
{
    return LPC1788_GPDMA_InitializeWorkspace_ChannelX(aWorkspace,
        GPDMA_CHANNEL7);
}
 ...
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_GPDMA_Stop(void *aWorkspace)
{
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_GPDMA_ChannelIsEnabled
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_GPDMA_Start(void *aWorkspace)
{
//    TUInt32 regValue = 0;
    T_LPC1788_GPDMA_Workspace *p = (T_LPC1788_GPDMA_Workspace *)aWorkspace;
    ...
    if (p->iControl == 0 || ETrue == LPC1788_GPDMA_ChannelIsEnabled(aWorkspace))
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_GPDMA_PrepareRequest
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *      T_uezError                  -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_GPDMA_PrepareRequest(
    void *aWorkspace,
    ...
    void *aISRCallbackWorkspace)
{
    ...
    // General output to match Table 675 from UM102347: LPC1788 user's manual
    flow = 0;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_GPDMA_BurstRequest
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_GPDMA_BurstRequest(
    void *aWorkspace,
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_GPDMA_SingleRequest
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                      if peripheral not supported.
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_GPDMA_SingleRequest(
    void *aWorkspace,
    T_gpdmaPeripheral aPeripheral)
{
    TUInt32 b;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_GPDMA_LastBurstRequest
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                      if peripheral not supported.
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_GPDMA_LastBurstRequest(
    void *aWorkspace,
    T_gpdmaPeripheral aPeripheral)
{
    TUInt32 b;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_GPDMA_LastSingleRequest
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *                                      if peripheral not supported.
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_GPDMA_LastSingleRequest(
    void *aWorkspace,
    T_gpdmaPeripheral aPeripheral)
{
    TUInt32 b;
...
/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_GPDMA_IsValidAddress
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
static TBool LPC1788_GPDMA_IsValidAddress(TUInt32 aAddr)
{
    // On the LPC1788, all addresses are valid
    return ETrue;
}
 ...
 *              service routine.
 * Inputs:
 *      T_LPC1788_GPDMA_Workspace *p      -- Port's definition to process interrupt
 *---------------------------------------------------------------------------*/
static void IGPDMAProcessInterrupt(T_LPC1788_GPDMA_Workspace *p)
{
...
const HAL_GPDMA LPC1788_GPDMA_Channel0_Interface = {
    {
        "GPDMA:LPC1788:CH0",
        0x0100,
        LPC1788_GPDMA_InitializeWorkspace_Channel0,
        sizeof(T_LPC1788_GPDMA_Workspace), },
    LPC1788_GPDMA_PrepareRequest,
    LPC1788_GPDMA_Start,
    LPC1788_GPDMA_Stop,
    LPC1788_GPDMA_IsValidAddress,

    // v2.04
    LPC1788_GPDMA_BurstRequest,
    LPC1788_GPDMA_SingleRequest,
    LPC1788_GPDMA_LastBurstRequest,
    LPC1788_GPDMA_LastSingleRequest };

const HAL_GPDMA LPC1788_GPDMA_Channel1_Interface = {
    {
        "GPDMA:LPC1788:CH1",
        0x0100,
        LPC1788_GPDMA_InitializeWorkspace_Channel1,
        sizeof(T_LPC1788_GPDMA_Workspace), },
    LPC1788_GPDMA_PrepareRequest,
    LPC1788_GPDMA_Start,
    LPC1788_GPDMA_Stop,
...
const HAL_GPDMA LPC1788_GPDMA_Channel2_Interface = {
    {
        "GPDMA:LPC1788:CH2",
        0x0100,
        LPC1788_GPDMA_InitializeWorkspace_Channel2,
        sizeof(T_LPC1788_GPDMA_Workspace), },
    LPC1788_GPDMA_PrepareRequest,
    LPC1788_GPDMA_Start,
    LPC1788_GPDMA_Stop,
...
const HAL_GPDMA LPC1788_GPDMA_Channel3_Interface = {
    {
        "GPDMA:LPC1788:CH3",
        0x0100,
        LPC1788_GPDMA_InitializeWorkspace_Channel3,
        sizeof(T_LPC1788_GPDMA_Workspace), },
    LPC1788_GPDMA_PrepareRequest,
    LPC1788_GPDMA_Start,
    LPC1788_GPDMA_Stop,
...
const HAL_GPDMA LPC1788_GPDMA_Channel4_Interface = {
    {
        "GPDMA:LPC1788:CH4",
        0x0100,
        LPC1788_GPDMA_InitializeWorkspace_Channel4,
        sizeof(T_LPC1788_GPDMA_Workspace), },
    LPC1788_GPDMA_PrepareRequest,
    LPC1788_GPDMA_Start,
    LPC1788_GPDMA_Stop,
...
const HAL_GPDMA LPC1788_GPDMA_Channel5_Interface = {
    {
        "GPDMA:LPC1788:CH5",
        0x0100,
        LPC1788_GPDMA_InitializeWorkspace_Channel5,
        sizeof(T_LPC1788_GPDMA_Workspace), },
    LPC1788_GPDMA_PrepareRequest,
    LPC1788_GPDMA_Start,
    LPC1788_GPDMA_Stop,
...
const HAL_GPDMA LPC1788_GPDMA_Channel6_Interface = {
    {
        "GPDMA:LPC1788:CH6",
        0x0100,
        LPC1788_GPDMA_InitializeWorkspace_Channel6,
        sizeof(T_LPC1788_GPDMA_Workspace), },
    LPC1788_GPDMA_PrepareRequest,
    LPC1788_GPDMA_Start,
    LPC1788_GPDMA_Stop,
...
const HAL_GPDMA LPC1788_GPDMA_Channel7_Interface = {
    {
        "GPDMA:LPC1788:CH7",
        0x0100,
        LPC1788_GPDMA_InitializeWorkspace_Channel7,
        sizeof(T_LPC1788_GPDMA_Workspace), },
    LPC1788_GPDMA_PrepareRequest,
    LPC1788_GPDMA_Start,
    LPC1788_GPDMA_Stop,
 ...
 *---------------------------------------------------------------------------*/
void LPC1788_GPDMA0_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE()
    ;
    HALInterfaceRegister("GPDMA0",
        (T_halInterface *)&LPC1788_GPDMA_Channel0_Interface, 0, 0);
}

void LPC1788_GPDMA1_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE()
    ;
    HALInterfaceRegister("GPDMA1",
        (T_halInterface *)&LPC1788_GPDMA_Channel1_Interface, 0, 0);
}

void LPC1788_GPDMA2_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE()
    ;
    HALInterfaceRegister("GPDMA2",
        (T_halInterface *)&LPC1788_GPDMA_Channel2_Interface, 0, 0);
}

void LPC1788_GPDMA3_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE()
    ;
    HALInterfaceRegister("GPDMA3",
        (T_halInterface *)&LPC1788_GPDMA_Channel3_Interface, 0, 0);
}

void LPC1788_GPDMA4_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE()
    ;
    HALInterfaceRegister("GPDMA4",
        (T_halInterface *)&LPC1788_GPDMA_Channel4_Interface, 0, 0);
}

void LPC1788_GPDMA5_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE()
    ;
    HALInterfaceRegister("GPDMA5",
        (T_halInterface *)&LPC1788_GPDMA_Channel5_Interface, 0, 0);
}

void LPC1788_GPDMA6_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE()
    ;
    HALInterfaceRegister("GPDMA6",
        (T_halInterface *)&LPC1788_GPDMA_Channel6_Interface, 0, 0);
}

void LPC1788_GPDMA7_Require(void)
{
    HAL_DEVICE_REQUIRE_ONCE()
    ;
    HALInterfaceRegister("GPDMA7",
        (T_halInterface *)&LPC1788_GPDMA_Channel7_Interface, 0, 0);
}
