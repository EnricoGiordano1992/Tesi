 ...
 *-------------------------------------------------------------------------*
 * Description:
 *      HAL implementation of the LPC1788 RTC Interface.
 * Implementation:
...
} T_LPC1788_RTC_Workspace;

/*---------------------------------------------------------------------------*
 * Routine:  LPC1788_RTC_InitializeWorkspace
 *---------------------------------------------------------------------------*
 * Description:
 *      Setup the LPC1788 RTC workspace.
 * Inputs:
 *      void *aW                    -- Particular RTC workspace
 * Outputs:
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_RTC_InitializeWorkspace(void *aWorkspace)
{
    T_LPC1788_RTC_Workspace *p = (T_LPC1788_RTC_Workspace *)aWorkspace;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_RTC_Get(void *aWorkspace, T_uezTimeDate *aTimeDate)
{
    TUInt32 d, t;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_RTC_Set(void *aWorkspace, const T_uezTimeDate *aTimeDate)
{
    T_LPC1788_RTC_Workspace *p = (T_LPC1788_RTC_Workspace *)aWorkspace;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_RTC_Configure(void *aWorkspace, TBool aIsExternalClock)
{
    T_LPC1788_RTC_Workspace *p = (T_LPC1788_RTC_Workspace *)aWorkspace;
 ...
 *      T_uezError                   -- Error code
 *---------------------------------------------------------------------------*/
T_uezError LPC1788_RTC_Validate(
        void *aWorkspace,
        const T_uezTimeDate *aTimeDate)
{
    ...
    LPC1788_RTC_Get(aWorkspace, &time);
    ...
    if (error == UEZ_ERROR_INVALID) {
        // Reset the RTC
        LPC1788_RTC_Set(aWorkspace, aTimeDate);
    }
 ...
 *---------------------------------------------------------------------------*/
const HAL_RTC LPC1788_RTC_Interface = {
        {
        "LPC1788:RTC",
        0x0100,
        LPC1788_RTC_InitializeWorkspace,
        sizeof(T_LPC1788_RTC_Workspace),
        },

        LPC1788_RTC_Get,
        LPC1788_RTC_Set,
        LPC1788_RTC_Validate, };

/*---------------------------------------------------------------------------*
 * Requirement routines:
 *---------------------------------------------------------------------------*/
void LPC1788_RTC_Require(TBool aIsExternalClock)
{
    ...
    HALInterfaceRegister(
            "RTC",
            (T_halInterface *)&LPC1788_RTC_Interface,
            0,
            &p_rtc);
    LPC1788_RTC_Configure(p_rtc, aIsExternalClock);
}
