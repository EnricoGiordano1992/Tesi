...
\title{Creazione ed analisi di una metodologia per la creazione di interfacce utente per Sistemi Embedded di fascia bassa}
%
%
...
\author{Enrico Giordano,~\IEEEmembership{Studente Magistrale,UniVR, VR386687}}
...
\begin{itemize}
\item \textit{dispositivi ciechi}, che non sono dotati di schermo e non permettono quindi di presentare un'interfaccia;
\item \textit{dispositivi visuali}, che sono dotati di schermo e permettono di presentare un'interfaccia.
\end{itemize}
...
\begin{itemize}
\item \textit{vincoli realtime}, ovvero il sistema deve rispondere entro un certo limite di tempo;
\item \textit{vincoli di memoria}, ovvero non eccedere con il codice compilato dal punto di vista di spazio occupato;
\item \textit{vincoli di consumo}, ovvero non si deve consumare troppa energia per eseguire un'operazione.
\end{itemize}

Rispettare questi vincoli è molto difficile, quindi solitamente si spende molto tempo e quindi risorse per progettare una \textit{business logic} che implementa la logica del circuito, ma è ancora più tedioso progettare un'interfaccia grafica per comandarla e controllarla.
...
\begin{itemize}
\item \textit{hard coded}, ovvero scrittura completamente manuale sia della grafica sia della business logic associata;
\item \textit{soft coded}, ovvero disegnando l'interfaccia il "GuiBuilder", programma in dotazione, per poi scrivere manualmente le funzioni associate all'interazione con i widget.  
\end{itemize}
...
Poichè si vuole dare una metodologia di sviluppo efficiente per la creazione di sistemi con interfaccia utente, è necessario l'utilizzo di un framework. Questo è stato creato a partire 
